	...
	u8 *arg1)
{
	...
	struct afe_tas2560_algo_params_t data_payload;
	...
	param_hdr.param_size = sizeof(struct afe_tas2560_algo_params_t);
	...
	result = afe_set_tas25xx_params(port_id, param_hdr, (u8 *)&data_payload);
	if (result) {
...
static int tas2560_algo_afe_get_param(uint32_t param_id, uint32_t module_id)
{
	...
	size = sizeof(struct afe_tas2560_algo_params_t);
	...
	result = afe_get_tas25xx_params(port_id, NULL, &param_hdr);
	if (result) {
