...
int tas2560_get_PPG(struct tas2560_priv *pTAS2560)
{
	...
	dev_dbg(pTAS2560->dev, "%s\n", __func__);
	return pTAS2560->mnPPG;
}

int tas2560_set_PPG(struct tas2560_priv *pTAS2560, int ppgvolume)
{
	int ret = -1;
	unsigned char *ppg = ppg_gain;

	dev_dbg(pTAS2560->dev, "%s", __func__);
	if ((ppgvolume > 110) || (ppgvolume < 0)) {
		dev_err(pTAS2560->dev, "invalid ppg volume, %d", ppgvolume);
		return ret;
	}
	pTAS2560->mnPPG = ppgvolume;
	ppg += ppgvolume * 4;
	dev_dbg(pTAS2560->dev, "PPG reg: 0x%x%x%x%x",
		ppg[0], ppg[1], ppg[2], ppg[3]);
	ret = pTAS2560->bulk_write(pTAS2560, TAS2560_PPG, ppg, 4);
	return ret;
}
int tas2560_parse_dt(struct device *dev, struct tas2560_priv *pTAS2560)
{
	...
	pTAS2560->mnSwitchGPIO2 = of_get_named_gpio(np,
		"ti,ear-switch-gpio2", 0);

	if (!gpio_is_valid(pTAS2560->mnSwitchGPIO2)) {
		dev_err(pTAS2560->dev, "Looking up %s property in node %s failed %d\n",
		"ti,ear-switch-gpio2", np->full_name, pTAS2560->mnSwitchGPIO2);
	} else {
		dev_err(pTAS2560->dev,  "ti,ear-switch-gpio2=%d",
			pTAS2560->mnSwitchGPIO2);
	}
...
static int tas2560_load_postpwrup(struct tas2560_priv *pTAS2560)
{
	...
	if ((pTAS2560->mnPPG >= 0) && (pTAS2560->mnPPG <= 110)) {
		ppg += pTAS2560->mnPPG * 4;
		pTAS2560->bulk_write(pTAS2560, TAS2560_PPG, ppg, 4);
	}
