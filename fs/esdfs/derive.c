...
{
	struct esdfs_inode_info *inode_i = ESDFS_I(d_inode(dentry));
	bool is_root;
...
void esdfs_set_derived_perms(struct inode *inode)
{
	...
	case ESDFS_TREE_MEDIA_PICS:
		i_gid_write(inode, AID_SDCARD_PICS);
		inode->i_mode |= 0770;
		break;

	case ESDFS_TREE_MEDIA_AV:
		i_gid_write(inode, AID_SDCARD_AV);
		inode->i_mode |= 0770;
		break;
...
int esdfs_derived_revalidate(struct dentry *dentry, struct dentry *parent)
{
	...
	if (ESDFS_I(parent->d_inode)->tree == ESDFS_TREE_ANDROID &&
	    ESDFS_INODE_CAN_LINK(d_inode(dentry)) &&
	    ESDFS_DENTRY_NEEDS_LINK(dentry) &&
...
int esdfs_derive_mkdir_contents(struct dentry *dir_dentry)
{
	...
	if (!d_inode(dir_dentry))
		return 0;

	inode_i = ESDFS_I(d_inode(dir_dentry));
	...
	if ((inode_i->tree != ESDFS_TREE_ANDROID_DATA &&
	     inode_i->tree != ESDFS_TREE_ANDROID_OBB) ||
	    (ESDFS_INODE_CAN_LINK(d_inode(dir_dentry)) &&
	...
	err = vfs_create(d_inode(lower_dir_path.dentry), lower_dentry, mode,
			 true);
	dput(lower_dentry);
