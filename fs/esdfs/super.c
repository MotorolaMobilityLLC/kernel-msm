...
static struct inode *esdfs_nfs_get_inode(struct super_block *sb, u64 ino,
					 u32 generation)
{
	struct super_block *lower_sb;
	struct inode *inode;
...
static struct dentry *esdfs_fh_to_dentry(struct super_block *sb,
...
static struct dentry *esdfs_fh_to_parent(struct super_block *sb,
					  struct fid *fid, int fh_len,
					  int fh_type)
{
...
const struct export_operations esdfs_export_ops = {
	.fh_to_dentry	   = esdfs_fh_to_dentry,
	.fh_to_parent	   = esdfs_fh_to_parent
};
