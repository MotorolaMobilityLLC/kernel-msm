				...
				union power_supply_propval *val)
{
	...
	if (!chg->step_chg_enabled) {
		val->intval = (int)chg->mmi.pres_chrg_step;
		return 0;
	}
...
static void typec_source_removal(struct smb_charger *chg)
{
	...
	/* clear USB ICL vote for ICL_LIMIT_VOTER */
	rc = vote(chg->usb_icl_votable, ICL_LIMIT_VOTER, false, 0);
	if (rc < 0)
		smblib_err(chg, "Error un-vote ICL from USB ICL rc=%d\n", rc);
