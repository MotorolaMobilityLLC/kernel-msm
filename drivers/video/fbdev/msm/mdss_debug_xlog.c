...
static void mdss_xlog_dump_all(void)
{
	...
	mdss_dbg_xlog.xlog_user_buf = kzalloc(max_size, GFP_KERNEL);
				...
				const char *name, ...)
{
	...
	/* if prev dump has not been read, drop the new one. */
	if (mdss_dbg_xlog.xlog_user_buf) {
		pr_info("previous dump exists and is not read\n");
		return;
	}
	...
	struct seq_file *s)
{
	...
	for (i = 0; i < len; i++) {
		seq_printf(s, "%p : %08x %08x %08x %08x\n", addr, reg_dump[i*4],
			reg_dump[i*4 + 1], reg_dump[i*4 + 2],
			reg_dump[i*4 + 3]);
		addr += 16;
	}
}

static void mdss_read_regs(struct mdss_debug_base *dbg, struct seq_file *s)
{
	struct range_dump_node *xlog_node, *xlog_tmp;
	int len;
	char *addr;
		...
		list_for_each_entry_safe(xlog_node, xlog_tmp,
			...
			if (xlog_node->reg_dump) {
				...
				if (len % 16)
					len += 16;
				len /= 16;
				regs_print_util(xlog_node->range_name, addr,
					len, xlog_node->reg_dump, s);
			}
		}
	} else {
		if (dbg->reg_dump) {
			...
			if (len % 16)
				len += 16;
			len /= 16;
			regs_print_util(dbg->name, addr, len,
				dbg->reg_dump, s);
		}
	}
}
...
static void debug_bus_print_util(u32 *reg_dump, struct seq_file *s, int len)
{
	...
	for (i = 0; i < len; i++) {
		seq_printf(s, "waddr=0x%x blk=%d tst=%d val=0x%x\n",
			reg_dump[i*4], reg_dump[i*4 + 1],
			reg_dump[i*4 + 2], reg_dump[i*4 + 3]);
	}
}

static void vbif_debug_bus_print_util(struct vbif_debug_bus *head,
	u32 *reg_dump, struct seq_file *s)
{
	int i, j;
	...
	for (i = 0; i < head->block_cnt; i++) {
		for (j = 0; j < head->test_pnt_cnt; j++) {
			seq_printf(s,
				...
				*(reg_dump), *(reg_dump+1),
				*(reg_dump+2), *(reg_dump+3));
			reg_dump += 4;
		}
	}
}

static void vbif_debug_bus_print(u32 *reg_dump, struct seq_file *s,
	u32 bus_size, struct vbif_debug_bus *dbg_bus)
{
	struct vbif_debug_bus *head;
	int i;
	int list_size = 0;
		...
		list_for_each_entry_safe(xlog_node, xlog_tmp,
			...
			if (xlog_node->reg_dump) {
				...
				if (len % 16)
					len += 16;
				len /= 16;
				dma_free_coherent(&mdata->pdev->dev, len * 16,
					xlog_node->reg_dump,
					xlog_node->regs_phys);
...
static void mdss_debugfs_xlog_free_mem(void)
{
	...
	if (mdss_dbg_xlog.enable_dbgbus_dump & MDSS_DBG_DUMP_IN_MEM) {
		if (mdss_dbg_xlog.dbgbus_dump) {
			dma_free_coherent(&mdata->pdev->dev,
				(mdata->dbg_bus_size * 4 * 4),
				mdss_dbg_xlog.dbgbus_dump,
				mdss_dbg_xlog.dbgbus_phy);
	...
	if (mdss_dbg_xlog.enable_vbif_dbgbus_dump & MDSS_DBG_DUMP_IN_MEM) {
		if (mdss_dbg_xlog.vbif_dbgbus_dump) {
			...
			for (i = 0; i < bus_size; i++) {
				head = dbg_bus + i;
				list_size +=
					(head->block_cnt * head->test_pnt_cnt);
			}
			/* 4 bytes * 4 entries for each test point */
			list_size *= 16;
			dma_free_coherent(&mdata->pdev->dev, list_size,
				mdss_dbg_xlog.vbif_dbgbus_dump,
				mdss_dbg_xlog.vbif_dbgbus_phys);
		...
		if (mdss_dbg_xlog.nrt_vbif_dbgbus_dump) {
			...
			list_size *= 16;
			dma_free_coherent(&mdata->pdev->dev, list_size,
				mdss_dbg_xlog.nrt_vbif_dbgbus_dump,
				mdss_dbg_xlog.nrt_vbif_dbgbus_phys);
...
static int mdss_debugfs_xlog_file_show(struct seq_file *s, void *v)
{
	...
	pr_info("%s: user space reading xlog\n", __func__);
	mutex_lock(&mdss_dbg_xlog.xlog_lock);
	if (mdss_dbg_xlog.enable_reg_dump & MDSS_DBG_DUMP_IN_MEM) {
		seq_puts(s,
			"====================REGS DUMP====================\n");
	...
	if (mdss_dbg_xlog.enable_dbgbus_dump & MDSS_DBG_DUMP_IN_MEM) {
		seq_puts(s,
			"==================DEBUG BUS DUMP==================\n");
		if (mdss_dbg_xlog.dbgbus_dump)
			debug_bus_print_util(mdss_dbg_xlog.dbgbus_dump,
				s, mdata->dbg_bus_size);
	}
	...
	if (mdss_dbg_xlog.enable_vbif_dbgbus_dump & MDSS_DBG_DUMP_IN_MEM) {
		if (mdss_dbg_xlog.vbif_dbgbus_dump) {
			seq_puts(s,
				"================VBIF DEBUG BUS DUMP================\n");
			vbif_debug_bus_print(mdss_dbg_xlog.vbif_dbgbus_dump, s,
				mdata->vbif_dbg_bus_size, mdata->vbif_dbg_bus);
		}
		if (mdss_dbg_xlog.nrt_vbif_dbgbus_dump) {
			seq_puts(s,
				"===============NRT VBIF DEBUG BUS DUMP================\n");
			vbif_debug_bus_print(mdss_dbg_xlog.nrt_vbif_dbgbus_dump,
				s, mdata->nrt_vbif_dbg_bus_size,
				mdata->nrt_vbif_dbg_bus);
		}
	}
	seq_puts(s, "===============XLOG===============\n");
	if (mdss_dbg_xlog.xlog_user_buf)
		seq_puts(s, mdss_dbg_xlog.xlog_user_buf);
	mutex_unlock(&mdss_dbg_xlog.xlog_lock);
	return 0;
}

static int mdss_debugfs_xlog_file_open(struct inode *inode,
	struct file *file)
{
	return single_open(file, mdss_debugfs_xlog_file_show,
		inode->i_private);
}

static int mdss_debugfs_xlog_file_release(struct inode *inode,
	struct file *file)
{
