...
static int dsi_mod_display_conf_to_edid(struct dsi_mod_display *pdata)
{
	...
	struct EDID edid = {
		...
		.timing_descriptors = {
			{
				.pixel_clock = (u16)(total_pixels * dsi_config->framerate / 10 / 1000),
				...
				.v_active_msb = (dsi_config->height >> 8) & 0x0f,
				.h_sync_offset_lsb = (u8)dsi_config->horizontal_front_porch,
				.h_sync_lsb = (u8)dsi_config->horizontal_pulse_width,
				...
				.v_size_lsb = (u8)dsi_config->physical_length_dim,
				.v_size_msb = (dsi_config->physical_length_dim >> 8) & 0x0f,
				.h_size_msb = (dsi_config->physical_width_dim >> 8) & 0x0f,
				.h_border = (u8)((dsi_config->horizontal_left_border +
						dsi_config->horizontal_right_border) / 2),
				.v_border = (u8)((dsi_config->vertical_top_border +
						dsi_config->vertical_bottom_border) / 2),
				.bitmask = 0x1a, /* TODO */
			}
		},
		.ext_block_count = 0,
