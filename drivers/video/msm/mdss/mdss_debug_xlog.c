	...
	u32 **dump_mem)
{
	...
	if (in_mem) {
		...
		if (*dump_mem) {
			dump_addr = *dump_mem;
			pr_debug("%s: start_addr:0x%p end_addr:0x%p\n",
...
static void mdss_dump_reg(const char *dump_name, u32 reg_dump_flag,
						  char *addr, int len, u32 **dump_mem, phys_addr_t *regs_phys)
{
	struct mdss_data_type *mdata = mdss_mdp_get_mdata();
	bool in_log, in_mem;
	u32 *dump_addr = NULL;
