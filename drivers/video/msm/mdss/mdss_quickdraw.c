	...
	struct mdss_quickdraw_buffer *mdss_buffer, int x, int y)
{
	...
	overlay.flags = 0;
	overlay.is_fg = 0;
	overlay.id = mdss_buffer->left_overlay_id;
	...
	if (dst_rect.w == 0 || dst_rect.h == 0)
		memset(&mdss_buffer->l_roi, 0, sizeof(struct mdp_rect));
	else {
		memcpy(&mdss_buffer->l_roi, &dst_rect, sizeof(struct mdp_rect));
		...
		mdss_buffer->left_overlay_id = overlay.id;
	}

	/* Handle split panel devices */
	if (mfd->panel_info->xres < mfd->fbi->var.xres) {
		memset(&overlay, 0, sizeof(struct mdp_overlay));
		overlay.src.width  = buffer->data.rect.w;
		overlay.src.height = buffer->data.rect.h;
		overlay.src.format = buffer->data.format;
		overlay.z_order = 0;
		overlay.alpha = 0xff;
		overlay.flags = MDSS_MDP_RIGHT_MIXER;
		overlay.is_fg = 0;
		overlay.id = mdss_buffer->right_overlay_id;
		fb_quickdraw_clip_rect(mfd->panel_info->xres,
			mfd->panel_info->yres, x - mfd->panel_info->xres, y,
			buffer->data.rect.w, buffer->data.rect.h, &src_rect,
			&dst_rect);
		overlay.src_rect.x = src_rect.x;
		overlay.src_rect.y = src_rect.y;
		overlay.src_rect.w = src_rect.w;
		...
		if (dst_rect.w == 0 || dst_rect.h == 0)
			memset(&mdss_buffer->r_roi, 0, sizeof(struct mdp_rect));
		else {
			memcpy(&mdss_buffer->r_roi, &dst_rect,
				sizeof(struct mdp_rect));
			...
			mdss_buffer->right_overlay_id = overlay.id;
		}
	}
	...
	struct mdss_quickdraw_buffer *mdss_buffer)
{
	...
	if (mdss_buffer->left_overlay_id == MSMFB_NEW_REQUEST &&
		mdss_buffer->right_overlay_id == MSMFB_NEW_REQUEST) {
		pr_err("%s: invalid buffer, no overlays set!\n", __func__);
		goto exit;
	}

	if (mdss_buffer->left_overlay_id != MSMFB_NEW_REQUEST) {
		ret = mdss_mdp_overlay_release_sub(mfd,
			mdss_buffer->left_overlay_id, true);
		mdss_buffer->left_overlay_id = MSMFB_NEW_REQUEST;
	}

	if (mdss_buffer->right_overlay_id != MSMFB_NEW_REQUEST) {
		ret = mdss_mdp_overlay_release_sub(mfd,
			mdss_buffer->right_overlay_id, true);
		mdss_buffer->right_overlay_id = MSMFB_NEW_REQUEST;
	}
	...
	struct mdss_quickdraw_buffer *mdss_buffer)
{
	...
	if (mdss_buffer->left_overlay_id != MSMFB_NEW_REQUEST) {
		...
		ovdata.id = mdss_buffer->left_overlay_id;
		ovdata.data.flags = 0;
	...
	if (mdss_buffer->right_overlay_id != MSMFB_NEW_REQUEST) {
		memset(&ovdata, 0, sizeof(ovdata));
		ovdata.id = mdss_buffer->right_overlay_id;
		ovdata.data.flags = 0;
		ovdata.data.offset = 0;
		ovdata.data.memory_id = buffer->mem_fd;
...
static int mdss_quickdraw_prepare(void *data, unsigned char panel_state)
{
	...
	reset_panel = mfd->quickdraw_reset_panel;
	...
	struct fb_quickdraw_buffer *buffer, int x, int y)
{
	...
	fb_quickdraw_clip_rect(mfd->fbi->var.xres, mfd->fbi->var.yres,
		x, y, w, h, NULL, &rect);
	...
	memcpy(&prim_commit.l_roi, &mdss_buffer->l_roi,
		sizeof(struct mdp_rect));
	memcpy(&prim_commit.r_roi, &mdss_buffer->r_roi,
		sizeof(struct mdp_rect));
...
static int mdss_quickdraw_erase(void *data, int x1, int y1, int x2, int y2)
{
	...
	fb_quickdraw_clip_rect(mfd->panel_info->xres, mfd->panel_info->yres,
		x1, y1, buffer->data.rect.w, buffer->data.rect.h,
		&src_rect, &dst_rect);
	if (dst_rect.w == 0 || dst_rect.h == 0)
		memset(&mdss_buffer->l_roi, 0, sizeof(struct mdp_rect));
	else
	...
	if (mfd->panel_info->xres < mfd->fbi->var.xres) {
		fb_quickdraw_clip_rect(mfd->panel_info->xres,
			mfd->panel_info->yres, x1 - mfd->panel_info->xres, y1,
			buffer->data.rect.w, buffer->data.rect.h, &src_rect,
			&dst_rect);
		if (dst_rect.w == 0 || dst_rect.h == 0)
			memset(&mdss_buffer->r_roi, 0, sizeof(struct mdp_rect));
