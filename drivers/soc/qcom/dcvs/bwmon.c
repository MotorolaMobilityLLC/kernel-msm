...
static ssize_t store_second_vote_en(struct kobject *kobj,
			struct attribute *attr, const char *buf,
			size_t count)
{
	struct hwmon_node *node = to_hwmon_node(kobj);
	...
	if (!val)
		...
		node->cur_freqs[1].ib = get_dst_from_map(hw,
							node->cur_freqs[0].ib);
	err = qcom_dcvs_update_votes(dev_name(hw->dev), node->cur_freqs,
					0x3, hw->dcvs_path);
	if (err < 0)
		dev_err(hw->dev, "second vote update failed: %d\n", err);
	mutex_unlock(&node->update_lock);

	hw->second_vote_en = val;
	return count;
}

static ssize_t show_second_vote_en(struct kobject *kobj,
			struct attribute *attr, char *buf)
{
	struct hwmon_node *node = to_hwmon_node(kobj);
	struct bw_hwmon *hw = node->hw;
...
static BWMON_ATTR_RW(second_vote_en);
...
static int qcom_bwmon_driver_probe(struct platform_device *pdev)
{
	...
	if (of_node) {
		...
		if (of_find_property(of_node, "qcom,secondary-map", &len)) {
		...
		else { // rely on bus width for conversion
	...
	//if (m->hw.second_dcvs_hw < NUM_DCVS_HW_TYPES) {
	if (m->hw.second_vote_en) {
