...
#ifdef CONFIG_MMI_LPM_DBG_MODULE
...
static int get_subsystem_sleep_stats(int i, struct sleep_stats *stat,
				const char *name)
{
	static uint64_t prev_duration[ARRAY_SIZE(subsystems)];
	u64 accumulated;
...
#ifdef CONFIG_MMI_LPM_DBG_MODULE
		sleep_time = get_subsystem_sleep_stats(i, tmp,
				subsystems[i].name);
		if (sleep_time < 0)
			sleep_time = 0;
