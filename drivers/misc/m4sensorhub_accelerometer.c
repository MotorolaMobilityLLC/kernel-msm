...
static void m4acc_data_callback(void *event_data, void *priv_data)
{
	...
	struct m4acc_driver_data *dd = iio_priv(iio);
	struct m4sensorhub_batch_sample *accel_data =
		(struct m4sensorhub_batch_sample *)event_data;

	mutex_lock(&(dd->mutex));

	if (dd->enable) {
		if (accel_data->cmd_id == M4SH_BATCH_CMD_FLUSH) {
			dd->iiodat.type = ACCEL_TYPE_EVENT_FLUSH;
		} else {
			dd->iiodat.type = ACCEL_TYPE_EVENT_DATA;
			dd->iiodat.event_data.x = accel_data->sensor_data[0];
			dd->iiodat.event_data.y = accel_data->sensor_data[1];
			dd->iiodat.event_data.z = accel_data->sensor_data[2];
			dd->iiodat.timestamp =
				ktime_to_ns(ktime_get_boottime());
		}
		dd->iiodat.seq_num = accel_data->seq;
		iio_push_to_buffers(iio, (unsigned char *)&(dd->iiodat));
	} else {
		m4acc_err("%s: Accel not enabled. Ignore event.\n", __func__);
	}
	mutex_unlock(&(dd->mutex));
	return;
}
