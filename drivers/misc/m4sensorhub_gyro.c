...
static void m4gyr_work_func(struct work_struct *work)
{
	...
	size = m4sensorhub_reg_getsize(dd->m4, M4SH_REG_GYRO_X);
	if (size < 0) {
	...
	err = m4sensorhub_reg_read(dd->m4, M4SH_REG_GYRO_X,
		(char *)&(dd->sensdat.x));
	if (err < 0) {
	...
	size = m4sensorhub_reg_getsize(dd->m4, M4SH_REG_GYRO_Y);
	if (size < 0) {
		m4gyr_err("%s: Reading from invalid register %d.\n",
			  __func__, size);
		err = size;
	...
	err = m4sensorhub_reg_read(dd->m4, M4SH_REG_GYRO_Y,
		(char *)&(dd->sensdat.y));
	if (err < 0) {
	...
	size = m4sensorhub_reg_getsize(dd->m4, M4SH_REG_GYRO_Z);
	if (size < 0) {
		m4gyr_err("%s: Reading from invalid register %d.\n",
			  __func__, size);
		err = size;
	...
	err = m4sensorhub_reg_read(dd->m4, M4SH_REG_GYRO_Z,
		(char *)&(dd->sensdat.z));
	if (err < 0) {
