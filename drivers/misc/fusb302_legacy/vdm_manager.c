...
int processVdmMessage(VdmManager * vdmm, SopType sop, u32 * arr_in,
	...
	if (vdm_type_in == STRUCTURED_VDM) {
		...
		case INITIATOR:
			...
			case DISCOVER_IDENTITY:
				{
					...
					arr_out[0] =
					    getBitsForStructuredVdmHeader
					    (svdmh_out);
					arr_out[1] =
					    getBitsForIdHeader(idh_out);
					arr_out[2] =
					    getBitsForCertStatVdo(csvdo_out);
					length_out = 3;
					...
					sendVdmMessage(vdmm, sop, arr_out,
						       length_out);
				}
				break;
			case DISCOVER_SVIDS:
				{
					...
					for (i = (*vdmm).last_svid_sent + 1;
					     (i < my_svids_info.num_svids)
					     && (i <
						 ((*vdmm).last_svid_sent + 1 +
						  MAX_SVIDS_PER_MESSAGE));
					     i++) {
			...
			case DISCOVER_MODES:
				{
					...
					my_modes_info =
					    (*vdmm).req_modes_info(svdmh_in.
								   svid);

					for (j = 0; j < my_modes_info.num_modes;
					     j++) {
						arr_out[j + 1] =
						    my_modes_info.modes[j];
						length_out++;
					}
			...
			case EXIT_MODE:{
					bool mode_exited;
					mode_exited =
					    (*vdmm).req_mode_exit(svdmh_in.svid,
								  svdmh_in.
								  obj_pos);
		...
		case RESPONDER_ACK:
			...
			case DISCOVER_IDENTITY:
				...
				// Try again? Delay, THEN try again? Simply report a failure?
				{
					...
					new_identity.id_header =
					    getIdHeader(arr_in[1]);
					new_identity.cert_stat_vdo =
					    getCertStatVdo(arr_in[2]);
					...
					if ((new_identity.id_header.
					     product_type == HUB)
					    || (new_identity.id_header.
						product_type == PERIPHERAL)
					    || (new_identity.id_header.
						product_type == AMA)) {
						new_identity.has_product_vdo =
						    true;
						new_identity.product_vdo = getProductVdo(arr_in[3]);	// !!! assuming it is before AMA VDO
					}

					if ((new_identity.id_header.
					     product_type == PASSIVE_CABLE)
					    || (new_identity.id_header.
						product_type == ACTIVE_CABLE)) {
						new_identity.has_cable_vdo =
						    true;
						new_identity.cable_vdo =
						    getCableVdo(arr_in[3]);
					}
					...
					if (sop == SOP) {
						...
						(*vdmm).inform_id(false, sop,
								  new_identity);
						return 1;
					}
					(*vdmm).inform_id(true, sop,
							  new_identity);
					return 0;
				}
				break;
			case DISCOVER_SVIDS:{
					...
					if (found_end != true) {
						...
						arr_out[0] =
						    getBitsForStructuredVdmHeader
						    (svdmh_out);
						sendVdmMessage(vdmm, sop,
							       arr_out, 1);
						// TODO: Wait for response
					} else {
						(*vdmm).inform_svids(found_end,
								     sop,
								     *num_sopstar_svids,
								     svid_arr);
					}
			...
			case DISCOVER_MODES:
				...
				// Try again? Delay, THEN try again? Simply report a failure?
				{
					...
					if (svdmh_in.cmd_type != RESPONDER_ACK)
						break;
					for (i = 1; i < length_in; i++) {
						my_modes_info.modes[i - 1] =
						    arr_in[i];
					}

					(*vdmm).inform_modes(true, sop,
							     my_modes_info);
			...
			case ENTER_MODE:
				...
				if (svdmh_in.cmd_type != RESPONDER_ACK) {
					(*vdmm).enter_mode_result(false,
								  svdmh_in.svid,
								  svdmh_in.
								  obj_pos);
				} else {
					(*vdmm).enter_mode_result(true,
								  svdmh_in.svid,
								  svdmh_in.
								  obj_pos);
				}
			...
			case EXIT_MODE:
				...
				// TODO: is that okay? - Gabe
				if (svdmh_in.cmd_type != RESPONDER_ACK) {
					(*vdmm).exit_mode_result(false,
								 svdmh_in.svid,
								 svdmh_in.
								 obj_pos);
				} else {
					(*vdmm).exit_mode_result(true,
								 svdmh_in.svid,
								 svdmh_in.
								 obj_pos);
				}
