...
static int32_t shub_probe_mag_unc(struct platform_device *pfdev);
static int32_t shub_remove_mag_unc(struct platform_device *pfdev);

#ifdef CONFIG_OF
    static struct of_device_id shub_of_match_tb_mag_unc[] = {
        { .compatible = "sharp,shub_mag_unc" ,},
        {}
    };
#else
    #define shub_of_match_tb_mag_unc NULL
#endif
...
#if 1
    SHUB_INPUT_VAL_CLEAR(shub_idev, ABS_HAT1X, data[INDEX_X]);
...
static struct platform_driver shub_mag_unc_driver = {
    .probe = shub_probe_mag_unc,
    .remove = shub_remove_mag_unc,
    .shutdown = NULL,
    .driver = {
        .name = "shub_dev_mag_unc",
        .of_match_table = shub_of_match_tb_mag_unc,
    },
};
...
static int32_t __init shub_init(void)
{
    ...
    ret = platform_driver_register(&shub_mag_unc_driver);
...
{
    platform_driver_unregister(&shub_mag_unc_driver);
}
