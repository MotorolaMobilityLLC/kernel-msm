...
static int32_t shub_probe_mag(struct platform_device *pfdev);
static int32_t shub_remove_mag(struct platform_device *pfdev);

#ifdef CONFIG_OF
    static struct of_device_id shub_of_match_tb_mag[] = {
        { .compatible = "sharp,shub_mag" ,},
        {}
    };
#else
    #define shub_of_match_tb_mag NULL
#endif
...
static struct platform_driver shub_mag_driver = {
    .probe = shub_probe_mag,
    .remove = shub_remove_mag,
    .shutdown = NULL,
    .driver = {
        .name = "shub_dev_mag",
        .of_match_table = shub_of_match_tb_mag,
    },
};
...
static int32_t __init shub_init(void)
{
    ...
    ret = platform_driver_register(&shub_mag_driver);
...
{
    platform_driver_unregister(&shub_mag_driver);
}
