...
enum hrtimer_restart pd_func_hrtimer(struct hrtimer *timer)
{
    ...
    FUSB_LOG("%s, ProtocolTimer=%d, PolicyStateTimer=%d, NoResponseTimer=%d, PRSwapTimer=%d\n",
...
void PolicySinkSelectCapability(void)
{
    ...
    } while ((PolicyState==peSinkSelectCapability)&&(oldPolicySubIndex<PolicySubIndex));
}
...
bool StoreUSBPDMessage(sopMainHeader_t Header, doDataObject_t* DataObject, bool transmitter, u8 SOPToken)
{
    int i, j, required;
    u8 header1;
