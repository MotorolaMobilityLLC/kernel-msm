...
typedef struct dhd_info {
	...
	s32 sar_enable;
#endif
	struct workqueue_struct *xmit_start_wq;
...
{
	struct dhd_rx_work *work;
	struct dhd_pub * pub;

	work = container_of(ptr, struct dhd_rx_work, work);
...
{
	struct dhd_start_xmit_work *work;
	int ret;
	dhd_info_t *dhd;
	struct dhd_bus * bus;

	work = container_of(ptr, struct dhd_start_xmit_work, work);
...
{
	struct dhd_start_xmit_work *start_xmit_work;
	dhd_info_t *dhd = DHD_DEV_INFO(net);

	start_xmit_work = (struct dhd_start_xmit_work*)
	...
	queue_work(dhd->xmit_start_wq, &start_xmit_work->work);
...
dhd_attach(osl_t *osh, struct dhd_bus *bus, uint bus_hdrlen)
{
	...
	dhd->xmit_start_wq = alloc_workqueue("bcmdhd-xmit-start-wq", WQ_HIGHPRI | WQ_UNBOUND | WQ_MEM_RECLAIM, 1);
...
#endif
	destroy_workqueue(dhd->xmit_start_wq);
