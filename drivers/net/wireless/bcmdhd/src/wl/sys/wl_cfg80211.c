 ...
 *
 * $Id: wl_cfg80211.c 538289 2015-03-03 07:07:36Z $
 */
/* */
#include <typedefs.h>
#include <linuxver.h>
	...
	struct cfg80211_ibss_params *params)
{
	...
	if (!bss) {
		if (IBSS_INITIAL_SCAN_ALLOWED == TRUE) {
			ssid.ssid_len = MIN(params->ssid_len,
					DOT11_MAX_SSID_LEN);
	...
	join_params.ssid.SSID_len = MIN(params->ssid_len, DOT11_MAX_SSID_LEN);
	...
	struct cfg80211_connect_params *sme)
{
	...
	if (chan) {
		...
		cfg->channel = ieee80211_frequency_to_channel(chan->center_freq);
		ssid.SSID_len = MIN(sme->ssid_len, DOT11_MAX_SSID_LEN);
	...
	struct beacon_parameters *info)
{
	...
	if ((ssid_ie = bcm_parse_tlvs((u8 *)&info->head[ie_offset],
		...
		if (dev_role == NL80211_IFTYPE_AP) {
			/* Store the hostapd SSID */
			memset(&cfg->hostapd_ssid.SSID[0], 0x00,
					DOT11_MAX_SSID_LEN);
			...
			memcpy(&cfg->hostapd_ssid.SSID[0], ssid_ie->data,
				...
				/* P2P GO */
			memset(&cfg->p2p->ssid.SSID[0], 0x00,
					DOT11_MAX_SSID_LEN);
