...
static int ufshid_set_flag(struct ufshid_dev *hid, u8 idn)
{
	...
	ret = ufshcd_query_flag_retry(hba, UPIU_QUERY_OPCODE_SET_FLAG, idn, 0,
				    &flag_result);
...
static int ufshid_clear_flag(struct ufshid_dev *hid, u8 idn)
{
	...
	ret = ufshcd_query_flag_retry(hba, UPIU_QUERY_OPCODE_CLEAR_FLAG, idn, 0,
				    NULL);
	if (ret) {
...
static inline int ufshid_issue_disable(struct ufshid_dev *hid)
{
	...
	if(hba->dev_info.wmanufacturerid == UFS_VENDOR_MICRON) {
	    ...
	    return ufshid_execute_query_op(hid, HID_OP_DISABLE, &attr_val);
	}
	return 0;
}
