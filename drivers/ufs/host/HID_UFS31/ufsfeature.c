...
const struct ufsf_offset ufsf_idn[] = {
	[DESC_IDN_DEV_IDX] = { 0xF0 },
	[DESC_IDN_DEV_3_1_IDX] = { 0x00 },
	[DESC_IDN_GEO_IDX] = { 0xF7 },
	[DESC_IDN_GEO_3_1_IDX] = { 0x07 },
};

const struct ufsf_offset ufsf_desc[] = {
	/* Device Descriptor */
	[DESC_DEV_MAX_SZ_IDX] = { 0xFF },
	[DESC_DEV_MAX_SZ_3_1_IDX] = { 0x65 },
	/* Configuration Descriptor */
	[DESC_CONF_MAX_SZ_IDX] =	{ 0xE6 },
	[DESC_CONF_MAX_SZ_3_1_IDX] = { 0xE6 },
	/* Unit Descriptor */
	[DESC_UNIT_MAX_SZ_IDX] = { 0x2D },
	[DESC_UNIT_MAX_SZ_3_1_IDX] = { 0x2D },
	/* Geometry Descriptor */
	[GEO_MAX_SZ_IDX] = { 0xFF },
	[GEO_MAX_SZ_3_1_IDX] = { 0x5E },
};

static inline u8 ufsf_get_idn(struct ufsf_feature *ufsf,
				enum ufsf_idn_idx name)
{
...
static int ufsf_read_dev_desc(struct ufsf_feature *ufsf, u8 selector)
{
	u8 *desc_buf;
	int ret;
	u32 max_size = ufsf_get_desc(ufsf, DESC_DEV_MAX_SZ_IDX);
	u32 idn = ufsf_get_idn(ufsf, DESC_IDN_DEV_IDX);
...
static int ufsf_read_geo_desc(struct ufsf_feature *ufsf, u8 selector)
{
	u8 *geo_buf;
	int ret;
	u32 max_size = ufsf_get_desc(ufsf, GEO_MAX_SZ_IDX);
	u32 idn = ufsf_get_idn(ufsf, DESC_IDN_GEO_IDX);
...
static void ufsf_read_unit_desc(struct ufsf_feature *ufsf, int lun, u8 selector)
{
	u8 *unit_buf;
	int lu_enable, ret;
	u32 max_size = ufsf_get_desc(ufsf, DESC_UNIT_MAX_SZ_IDX);
...
/* it is a function only for ufsf hid */
static inline void ufsf_on_idle_schdule_work(struct ufs_hba *hba,
...
check_last_req:
	ufsf_on_idle_schdule_work(hba, lrbp);
...
inline void ufsf_event_notify(struct ufs_hba *hba, enum ufs_event_type evt,
		u32 data)
{
...
{
	struct ufsf_feature *ufsf = ufs_qcom_get_ufsf(hba);
	int sel = ufsf->samsung_sel;
	int ret;
	int retries;

	for (retries = 0; retries < QUERY_REQ_RETRIES; retries++) {
		ret = ufsf_query_flag(hba, opcode, idn, index, sel, flag_res);
		if (ret)
			dev_dbg(hba->dev,
