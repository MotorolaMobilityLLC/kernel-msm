...
const struct ufshid_offset ufshid_idn[] = {
	...
	/* Attribute */
	[QUERY_ATTR_IDN_HID_OP_IDX] = { 0x80 },
	[QUERY_ATTR_IDN_HID_OP_3_1_IDX] = { 0x20 },
	[QUERY_ATTR_IDN_HID_FRAG_LV_IDX] = { 0x81 },
	[QUERY_ATTR_IDN_HID_FRAG_LV_3_1_IDX] = { 0x21 },
	[HID_SEPARATION_BOUNDARY] = { 0x00 },
	/* UFS 4.0 / 3.1 Use Common HID IDN */
	/* Attribute */
	[QUERY_ATTR_IDN_HID_SZ_IDX] = { 0x8A },
	[QUERY_ATTR_IDN_HID_AVAIL_SZ_IDX] = { 0x8B },
	[QUERY_ATTR_IDN_HID_PROGRESS_RATIO_IDX] = { 0x8C },
	[QUERY_ATTR_IDN_HID_STATE_IDX] = { 0x8D },
	[QUERY_ATTR_IDN_HID_L2P_FRAG_LV_IDX] = { 0x8E },
	[QUERY_ATTR_IDN_HID_L2P_DEFRAG_TH_IDX] = { 0x8F },
	[QUERY_ATTR_IDN_HID_FEAT_SUP_IDX] = { 0x90 },
};

const struct ufshid_offset ufshid_desc[] = {
	/* UFS 4.0 / 3.1 Use Separate HID Dev/Geometry Desc */
	/* Device */
	[DEV_DESC_PARAM_HID_VER_IDX] = { 0xF7 },
	[DEV_DESC_PARAM_HID_VER_3_1_IDX] = { 0x59 },
	/* Geometry */
	[GEO_DESC_HID_MAX_LBA_RANGE_CNT_IDX] = { 0xF8 },
	[GEO_DESC_HID_MAX_LBA_RANGE_CNT_3_1_IDX] = { 0x5D },
	[GEO_DESC_HID_MAX_LBA_RANGE_SZ_IDX] = { 0xF9 },
	[GEO_DESC_HID_MAX_LBA_RANGE_SZ_3_1_IDX] = { 0x59 },
};
...
static int ufshid_read_attr(struct ufshid_dev *hid, enum ufshid_idn_idx name,
			    u32 *attr_val)
{
...
static int ufshid_write_attr(struct ufshid_dev *hid, enum ufshid_idn_idx name,
			     u32 val)
{
	struct ufs_hba *hba = hid->ufsf->hba;
	struct ufsf_feature *ufsf = hid->ufsf;
...
static int ufshid_read_attr_micron(struct ufshid_dev *hid, u8 idn, u32 *attr_val)
{
	struct ufs_hba *hba = hid->ufsf->hba;
	int ret = 0;
...
void ufshid_get_geo_info(struct ufsf_feature *ufsf, u8 *geo_buf)
{
	...
	max_size_offset = ufshid_get_desc(hid, GEO_DESC_HID_MAX_LBA_RANGE_SZ_IDX);
	max_cnt_offset = ufshid_get_desc(hid, GEO_DESC_HID_MAX_LBA_RANGE_CNT_IDX);
...
void ufshid_get_dev_info(struct ufsf_feature *ufsf, u8 *desc_buf)
{
	...
	ver_offset = ufshid_get_desc(ufsf->hid_dev, DEV_DESC_PARAM_HID_VER_IDX);
...
{
	if (ufshid_write_attr(hid, QUERY_ATTR_IDN_HID_OP_IDX, op))
		return -EINVAL;

	msleep(200);

	if (ufshid_read_attr(hid, QUERY_ATTR_IDN_HID_FRAG_LV_IDX, attr_val))
		return -EINVAL;
				       ...
				       u32 *attr_val)
{
	...
	return ufshid_write_attr(hid, QUERY_ATTR_IDN_HID_OP_IDX, op);
...
#if defined(CONFIG_MICRON_UFS31_HID)
static int ufshid_get_analyze_and_issue_execute_micron(struct ufshid_dev *hid)
{
	u32 attr_val;
	int frag_level;

	//get micron ufs frag level
	if (ufshid_read_attr_micron(hid, QUERY_ATTR_IDN_HID_FRAG_STATUS_MICRON_UFSV4, &frag_level))
		return -EINVAL;
	//get micron ufs hid execution progress
	if (ufshid_read_attr_micron(hid, QUERY_ATTR_IDN_HID_PROGRESS_MICRON_UFSV4, &attr_val))
		return -EINVAL;
	...
	if (attr_val != HID_PROG_ONGOING) {
		if(frag_level!= HID_LEV_GREEN_MICRON) {
			ufshid_set_flag(hid, QUERY_FLAG_IDN_HID_EN_MICRON_UFSV4);
			return HID_REQUIRED;
		} else {
...
static int ufshid_analyze_and_get_attr(struct ufshid_dev *hid,
				       enum ufshid_idn_idx name, u32 *attr_val)
{
	int ret;
	u32 hid_state;
	...
	if (ufshid_write_attr(hid, QUERY_ATTR_IDN_HID_OP_IDX, HID_OP_ANALYZE))
		return -EINVAL;

	msleep(200);

	if (ufshid_read_attr(hid, QUERY_ATTR_IDN_HID_STATE_IDX, &hid_state))
		return -EINVAL;
...
static bool ufshid_is_in_progress(struct ufshid_dev *hid)
{
	...
	ret = ufshid_read_attr(hid, QUERY_ATTR_IDN_HID_STATE_IDX, &state);
	HID_DEBUG(hid, "ufshid_is_in_progress ret (%d), state = (%d)", ret,state);
	if (ret)
		return false;
...
static bool ufshid_hid_progress_chk_in_work(struct ufshid_dev *hid)
{
	...
	ret = ufshid_read_attr(hid, QUERY_ATTR_IDN_HID_PROGRESS_RATIO_IDX,
			       &attr_val);
	...
	HID_DEBUG(hid, "HID is ended or err (%d), so trigger off, hid_trigger =%d", ret,hid->hid_trigger);

	ufshid_issue_disable(hid);
	msleep(200);

	ret = ufshid_analyze_and_get_attr(hid,
					QUERY_ATTR_IDN_HID_FRAG_LV_IDX, &attr_val);
	if (!ret)
		HID_DEBUG(hid, "Frag_lv %d Frag_mode %d Frag_stat %d HID_need_exec %d",
...
static void micron_ufshid_trigger_work_fn(struct work_struct *dwork)
{
	...
	ret = ufshid_get_analyze_and_issue_execute_micron(hid);
...
static void ufshid_init_attr(struct ufshid_dev *hid)
{
	...
	ufshid_read_attr(hid, QUERY_ATTR_IDN_HID_FEAT_SUP_IDX, &attr_val);
	...
	if (hid->l2p_defrag_sup) {
		...
		ufshid_write_attr(hid, QUERY_ATTR_IDN_HID_L2P_DEFRAG_TH_IDX,
				  hid->l2p_defrag_threshold);
	}

	hid->hid_size = HID_SIZE_DEFAULT;
	ufshid_write_attr(hid, QUERY_ATTR_IDN_HID_SZ_IDX, hid->hid_size);
}
...
static void ufshid_restore_attr(struct ufshid_dev *hid)
{
	...
	u8 idn = ufshid_get_idn(hid, QUERY_ATTR_IDN_HID_L2P_DEFRAG_TH_IDX);
	int ret;
					   ...
					   const char *buf, size_t count)
{
	...
	ret = ufshid_write_attr(hid, QUERY_ATTR_IDN_HID_SZ_IDX, val);
	if (ret)
		return -EINVAL;
...
static ssize_t ufshid_sysfs_show_l2p_frag_lvl(struct ufshid_dev *hid, char *buf)
{
	...
	ret = ufshid_analyze_and_get_attr(hid,
					  QUERY_ATTR_IDN_HID_L2P_FRAG_LV_IDX,
					  &attr_val);
	if (ret) {
						       ...
						       size_t count)
{
	...
	ret = ufshid_write_attr(hid, QUERY_ATTR_IDN_HID_L2P_DEFRAG_TH_IDX, val);
	if (ret)
		return -EINVAL;
...
static ssize_t do_ufshid_sysfs_show_color_samsung(struct ufshid_dev *hid, char *buf)
{
	u32 attr_val;
	int frag_level;
	bool param_mode;
	...
	if (hid->hid_trigger) {
		INFO_MSG("HID is in progress...");
		return snprintf(buf, PAGE_SIZE, "YELLOW \n");
	}
	...
	if (spec_ver >= HID_3_0_VER) {
		...
		if (ret) {
			if (attr_val == RESULT_NOT_DEFRAG_REQUIRED) {
				INFO_MSG("Defrag is not required...");
				return snprintf(buf, PAGE_SIZE,"GREEN\n");
			}
			goto err;
		}
...
#if defined(CONFIG_MICRON_UFS31_HID)
static ssize_t do_ufshid_sysfs_show_color_micron(struct ufshid_dev *hid, char *buf)
{
	u32 attr_val;
	int frag_level;

	if (ufshid_read_attr_micron(hid, QUERY_ATTR_IDN_HID_FRAG_STATUS_MICRON_UFSV4, &attr_val))
		return -EINVAL;
	frag_level = attr_val;
						...
						char *buf)
{
	...
	if (ufshid_read_attr(hid, QUERY_ATTR_IDN_HID_PROGRESS_RATIO_IDX, &attr_val))
		return -EINVAL;
						...
						char *buf)
{
	...
	if (ufshid_read_attr(hid, QUERY_ATTR_IDN_HID_AVAIL_SZ_IDX, &attr_val))
		return -EINVAL;
...
static ssize_t ufshid_sysfs_show_hid_state(struct ufshid_dev *hid, char *buf)
{
	...
	if (ufshid_read_attr(hid, QUERY_ATTR_IDN_HID_STATE_IDX, &attr_val))
		return -EINVAL;
...
static ssize_t ufshid_sysfs_show_debug_op(struct ufshid_dev *hid, char *buf)
{
	...
	if (ufshid_read_attr(hid, QUERY_ATTR_IDN_HID_OP_IDX, &attr_val))
		return -EINVAL;
					   ...
					   const char *buf, size_t count)
{
	...
	if (ufshid_write_attr(hid,QUERY_ATTR_IDN_HID_OP_IDX, val))
		return -EINVAL;
