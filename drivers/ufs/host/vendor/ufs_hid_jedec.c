...
static int ufs_update_hid_support(struct ufs_hba *hba)
{
 	unsigned char desc_buf[QUERY_DESC_MAX_SIZE];
 	int err = 0;
	struct ufs_qcom_host *ufs = ufshcd_get_variant(hba);

 	pr_info("ufs_sysfs_hid_is_visible\n");
 	...
 	dev_err(hba->dev,"%s:  get hid jedec support = %d \n",__func__, ufs->hid_jedec.hid_sup );
...
int moto_hid_jedec_init(struct ufs_hba *hba)
{
	...
	/* for < kernel 6.12 it need get hid support status */
	err = ufs_update_hid_support(hba);
	if (err) {
	 	dev_err(hba->dev, "%s: Failed to get ufs hid support status\n",__func__);
	 	return 0;
	}
