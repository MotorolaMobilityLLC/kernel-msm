...
#ifdef CONFIG_UFS_JEDEC_HID
	INFO_MSG("run reset_host.. hid_state(%d) -> JEDEC_HID_RESET",
...
{
	*err = ufsf_prep_fn(ufs_host_get_ufsf(hba), lrbp);
}
		...
		u32 data)
{
	...
	if (evt == UFS_EVT_DEV_RESET && val == 0)
		 ...
		 */
		ufsf_reset_lu(ufs_host_get_ufsf(hba));

	if (evt == UFS_EVT_WL_SUSP_ERR)
		ufsf_resume(ufs_host_get_ufsf(hba), true);
}
