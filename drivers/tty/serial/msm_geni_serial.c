...
static bool msm_serial_try_disable_interrupts(struct uart_port *uport)
{
	...
	/*
	 * Ensure register writes goes through before returning
	 * interrupt status.
	 */
	mb();
 ...
 */
bool geni_wait_for_cmd_done(struct uart_port *uport, bool interrupts_masked)
{
			      ...
			      unsigned int count)
{
	...
	if (!locked) {
		...
		interrupts_masked = msm_serial_try_disable_interrupts(uport);
		geni_cancel_m_cmd(uport->membase);

		timeout = geni_wait_for_cmd_done(uport, interrupts_masked);
		if (timeout) {
...
static int msm_geni_serial_prep_dma_tx(struct uart_port *uport)
{
	...
	bool timeout, interrupts_masked;
	int ret = 0;
