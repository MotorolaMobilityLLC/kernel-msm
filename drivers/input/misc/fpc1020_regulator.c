...
int fpc1020_io_regulator_configure(fpc1020_data_t *fpc1020)
{
	int error = 0;
	...
	if (IS_ERR(fpc1020->vdd_io)) {
		...
		dev_err(&fpc1020->spi->dev,
			"io Regulator get failed, vdd_io, error=%d\n", error);
		goto supply_err;
	}
	...
	if (regulator_count_voltages(fpc1020->vdd_io) > 0) {
		...
		if (error) {
			dev_err(&fpc1020->spi->dev,
				"io regulator set(tx) failed, error=%d\n", error);
			goto supply_err;
		}
	}
...
int fpc1020_io_regulator_release(fpc1020_data_t *fpc1020)
{
...
int fpc1020_io_regulator_set(fpc1020_data_t *fpc1020, bool enable)
{
	int error = 0;
