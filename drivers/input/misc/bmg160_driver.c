...
static ssize_t bmg_place_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
...
static ssize_t bmg_place_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	unsigned long data;
...
static DEVICE_ATTR(place, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bmg_place_show, bmg_place_store);
#ifdef BMG_DEBUG
...
static int bmg_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (client->dev.of_node) {
		...
		err = bmg_parse_dt(&client->dev, client_data->bst_pd);
		if (err) {
