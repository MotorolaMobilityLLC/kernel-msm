	...
	switch (cmd) {
		...
		case MMC3X30_IOC_RESET:
	 	...
	 	case MMC3X30_IOC_READ_REG:
			...
			printk("mmc3x30 Read register 0x%0x\n",reg_addr);
	 	...
	 	case MMC3X30_IOC_WRITE_REG:
			...
			rc = regmap_write(mmc3x30_data_struct->regmap, data[1], data[0]);	
			if (rc) {
				dev_err(&mmc3x30_data_struct->i2c->dev, "write reg %d failed at %d.(%d)\n",
						       data[1], __LINE__, rc);
				return -EFAULT;
			}
			printk("mmc3x30 Write '0x%0x' to  register 0x%0x\n", data[0], data[1]);
			break;   
	 	...
	 	case MMC3X30_IOC_READ_REGS:
			...
			rc = regmap_bulk_read(mmc3x30_data_struct->regmap, reg_addr, data, reg_num);
			if (rc) {
				dev_err(&mmc3x30_data_struct->i2c->dev, "read reg %d failed at %d.(%d)\n",
						         reg_addr, __LINE__, rc);
			}
		
			printk("mmc3x30 data: %x %x %x \n%x %x %x\n", data[0], data[1], data[2], data[3], data[4], data[5]);	
...
#ifdef CONFIG_COMPAT
static long mmc3x30_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
		case COMPAT_MMC3X30_IOC_SET:
			ret = file->f_op->unlocked_ioctl(file, MMC3X30_IOC_SET, (unsigned long)arg64);
			if(ret < 0)
			{
		...
		case COMPAT_MMC3X30_IOC_RESET:
			ret = file->f_op->unlocked_ioctl(file, MMC3X30_IOC_RESET, (unsigned long)arg64);
			if(ret < 0)
			{
		...
		case COMPAT_MMC3X30_IOC_READ_REG:
			ret = file->f_op->unlocked_ioctl(file, COMPAT_MMC3X30_IOC_READ_REG, (unsigned long)arg64);
			if(ret < 0)
			{
		...
		case COMPAT_MMC3X30_IOC_WRITE_REG:
			ret = file->f_op->unlocked_ioctl(file, COMPAT_MMC3X30_IOC_WRITE_REG, (unsigned long)arg64);
			if(ret < 0)
			{
		...
		case COMPAT_MMC3X30_IOC_READ_REGS:
			ret = file->f_op->unlocked_ioctl(file, COMPAT_MMC3X30_IOC_READ_REGS, (unsigned long)arg64);
			if(ret < 0)
			{
				...
				struct device_attribute *attr, char *buf)
{
	...
	else if(mmc3x30_data_struct->device_id == MMC3630_DEVICE_ID)
	{
		...
		rc = regmap_write(mmc3x30_data_struct->regmap, reg_addr, data[0]);	
		if (rc) {
			dev_err(&mmc3x30_data_struct->i2c->dev, "write reg %d failed at %d.(%d)\n",
						       reg_addr, __LINE__, rc);
		}
...
static ssize_t mmc3x30_value_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	int ret;
	//s8 *tmp;
	struct mmc3x30_vec vec;
...
static ssize_t mmc3x30_set_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int rc = -1;
	struct mmc3x30_data *memsic = dev_get_drvdata(dev);
	...
	msleep(1);//setting time
	return count;//nothing return
}

static ssize_t mmc3x30_reset_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
...
static int mmc3x30_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	int res = 0;
	struct mmc3x30_data *memsic;
	...
	if(res){
		...
		printk("mmc3x30_device initial OK\n");
	}
#endif
