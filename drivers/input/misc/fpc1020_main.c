...
static int fpc1020_probe(struct spi_device *spi)
{
...
#if 0
	error = gpio_request( reset_gpio, NULL );
	if( error )
	{
	...
	if (error)
		goto err;
printk(KERN_ERR "fpc1020_reset\n");
	error = fpc1020_check_hw_id(fpc1020);
	if (error)
		goto err;
printk(KERN_ERR "fpc1020_hw_id\n");
					...
					struct fpc1020_platform_data *pdata)
{
	...
	printk(KERN_ERR "%s Test Log :  pdata->irq_gpio : %d pdata->reset_gpio : %d pdata->cs_gpio : %d pdata->use_fpc2050 : %d force_hwid : 0x%x !!!!\n"
			,__func__,pdata->irq_gpio, pdata->reset_gpio, pdata->cs_gpio, pdata->use_fpc2050, pdata->force_hwid);
