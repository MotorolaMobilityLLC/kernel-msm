...
extern unsigned int key_swap_algo(unsigned int code, unsigned int status);
#else
unsigned int __attribute__((weak)) key_swap_algo(unsigned int code, unsigned int status)
{
	pr_debug("%s(), code = %u\n", __func__, code);
	return code;
}
