...
int synaptics_rmi4_i2c_read_extern(struct synaptics_rmi4_data *rmi4_data,
		unsigned short addr, unsigned char *data, unsigned short length)
{
return synaptics_rmi4_i2c_read(rmi4_data,addr,data,length);
						...
						*rmi4_data, bool on)
{
    ...
    if (gpio_is_valid(rmi4_data->board->power_ldo_gpio)) {
			...
			if (retval) {
				printk("rmi4 power ldo gpio request failed\n");
				return retval;
			}
			retval = gpio_direction_output(rmi4_data->board->power_ldo_gpio, 1);
			if (retval) {
				printk("rmi4 set_direction for power ldo gpio failed\n");
				goto free_ldo_gpio;
			}
		}	
    #else
...
err_gpio_config:
    if (rmi4_data->ts_pinctrl){
	pinctrl_put(rmi4_data->ts_pinctrl);//free pinctrl
    }
