	...
	struct synaptics_rmi4_func_packet_regs *regs)
{
	...
	ii = 1;
	for (r = 0, r_offset = 0; ii < sz && r < regs->nr_regs; ++ii) {
		for (jj = 0, mask = 1; jj < 8 &&
			r < regs->nr_regs;
				++jj, ++r, mask <<= 1) {
			...
			if ((reg->offset >= 0) != reg->expected) {
				pr_err("touch register error: r%d is%s"
					" present, but was%s expected\n",
					r,
					reg->offset >= 0 ? "" : " NOT",
					reg->expected ? "" : " NOT");
			}
	...
	for (r = 0, ii = 0; r < regs->nr_regs && ii < sz; ++r) {
		...
		expected_reg_size = 0;
		for (s = 0; s < reg->nr_subpkts && ii < sz;) {
			#define NR_SUBPKT_PRESENCE_BITS 7
			for (jj = 0, mask = 1;
				jj < NR_SUBPKT_PRESENCE_BITS &&
				s < reg->nr_subpkts && ii < sz;
					++jj, ++s, mask <<= 1) {
				struct synaptics_rmi4_subpkt *subpkt =
							reg->subpkt + s;
				subpkt->present = (data[ii] & mask) != 0;

				if (subpkt->present)
					expected_reg_size += subpkt->size;

				if (subpkt->present != subpkt->expected)
	...
	struct synaptics_rmi4_func_packet_regs *regs, unsigned char r)
{
	...
	for (ii = 0, offset = 0; ii < reg->nr_subpkts; ii++) {
...
static int synaptics_dsx_panel_sync_cb(struct notifier_block *nb,
		unsigned long event, void *ignore)
{
	...
	case MMI_PANEL_EVENT_PWR_ON:
			return 1; /* not interested at this time */
	}
	atomic_set(&rmi4_data->panel_off_flag, value);
	pr_debug("panel_off %d\n", value);
	return 0;
}
				...
				struct synaptics_rmi4_data *rmi4_data)
{
	...
	of_property_read_u32(np, "synaptics,panel-sync-enabled",
			 &panel_sync_enabled);
	rmi4_data->panel_sync_en = (bool)panel_sync_enabled;
...
*/
	...
	if (rmi4_data->panel_sync_en) {
		rmi4_data->panel_nb.notifier_call = synaptics_dsx_panel_sync_cb;
...
static void synaptics_rmi4_late_resume(struct early_suspend *h)
{
	...
	/* FIXME reset is only here to overcome IC misbehavior */
