			...
			struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "invalied rw_flag %d\n", rw_flag);
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!token) {
		...
		rw_addr = (u16)result;
		ts_err("rw addr is 0x%x\n", rw_addr);
	}

	/* get length */
	token = strsep(&pos, ":");
	if (!token) {
		ts_err("invalied length info\n");
		goto err_out;
	} else {
	...
	if (rw_flag == 2) {
		...
		ts_err("%s write to addr (%x) with data %*ph\n",
...
static int goodix_ts_power_init(struct goodix_ts_core *core_data)
{
	...
	if (regulator_count_voltages(core_data->iovdd) > 0) {
...
int goodix_ts_power_on(struct goodix_ts_core *core_data)
{
	...
	if (core_data->iovdd) {
		...
		if (!r) {
			ts_info("regulator iovdd enable SUCCESS");
			if (ts_bdata->power_on_delay_us)
				usleep_range(ts_bdata->power_on_delay_us,
						ts_bdata->power_on_delay_us);
		} else {
...
int goodix_ts_power_off(struct goodix_ts_core *core_data)
{
	...
	if (core_data->iovdd) {
		...
		if (!r) {
			ts_info("regulator iovdd disable SUCCESS");
			if (ts_bdata->power_off_delay_us)
				usleep_range(ts_bdata->power_off_delay_us,
						ts_bdata->power_off_delay_us);
		} else {
...
/**
 * goodix_ts_power_release - Get regulator for touch device
 * @core_data: pointer to touch core data
 * return: 0 ok, <0 failed
 */
static int goodix_ts_power_release(struct goodix_ts_core *core_data)
{
	struct device *dev = NULL;
	struct goodix_ts_board_data *ts_bdata;
	int r = 0;

	ts_info("Power release");
	/* dev:i2c client device or spi slave device*/
	dev =  core_data->ts_dev->dev;
	ts_bdata = board_data(core_data);
...
static int goodix_ts_suspend(struct goodix_ts_core *core_data)
{
	...
	if (!list_empty(&goodix_modules.head)) {
		list_for_each_entry(ext_module, &goodix_modules.head, list) {
			...
			r = ext_module->funcs->before_suspend(
	...
	if (!list_empty(&goodix_modules.head)) {
		list_for_each_entry(ext_module, &goodix_modules.head, list) {
			...
			r = ext_module->funcs->after_suspend(
				core_data, ext_module);
			if (r == EVT_CANCEL_SUSPEND) {
				mutex_unlock(&goodix_modules.mutex);
				ts_info("Canceled by module:%s",
...
static int goodix_ts_resume(struct goodix_ts_core *core_data)
{
	...
	if (!list_empty(&goodix_modules.head)) {
		list_for_each_entry(ext_module, &goodix_modules.head, list) {
			...
			r = ext_module->funcs->after_resume(
				core_data, ext_module);
			if (r == EVT_CANCEL_RESUME) {
				mutex_unlock(&goodix_modules.mutex);
				ts_info("Canceled by module:%s",
	...
	unsigned long event, void *data)
{
	...
	if (fb_event && fb_event->data && core_data) {
		if (event == FB_EARLY_EVENT_BLANK) {
			...
			ts_info("blank's event = %ld", *blank);
