...
static int Upgrade_FW_CFG(void)
{
	...
	fw_fd = filp_open("/sdcard/it7260.fw", O_RDONLY, 0);
	if (fw_fd < 0)
		printk("open /sdcard/it7260.fw failed\n");
	else
		fw_size = fw_fd->f_op->read(fw_fd, fw_buf, 0x8000, &fw_fd->f_pos);
	printk("fw_ver : %x,%x,%x,%x\n",fw_buf[8], fw_buf[9], fw_buf[10], fw_buf[11]);
	printk("--------------------- fw_size = %x\n", fw_size);

	config_fd = filp_open("/sdcard/it7260.cfg", O_RDONLY, 0);
	if (config_fd < 0)
		printk("open /sdcard/it7260.cfg failed\n");
	else
