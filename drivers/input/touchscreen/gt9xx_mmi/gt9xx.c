...
static void goodix_ts_sensor_report(struct goodix_ts_data *ts)
{
	...
	if (touch_num) {
		for (i = 0; i < touch_num; i++) {
			...
			id = coor_data[0];
			if (ts->pdata->with_pen && (id & MASK_BIT_8) && !input_w) {
				dev_dbg(&ts->client->dev,
...
{
	if (ts->pdata->enable_int_as_output)
...
/*******************************************************
Function:
	parse platform infomation form devices tree.
Input:
		...
		struct goodix_ts_platform_data *pdata)
{
	...
	/* can select the i2c address by controller */
	pdata->enable_int_as_output = of_property_read_bool(np,
