...
int hx_drm_notifier_callback(struct notifier_block *self,
...
//    uint8_t buf[128];
    uint8_t *buf1;
    buf1 = kzalloc(128*sizeof(uint8_t),GFP_KERNEL);
    if(buf1 == NULL)
    {
        goto BUF_ALLOC_FAIL;
    }
//    memset(buf,0x00,sizeof(buf));
    ...
    ts_status = himax_touch_get(ts, buf1, ts_path, ts_status);
    if (ts_status == HX_TS_GET_DATA_FAIL)
        goto END_FUNCTION;

    ts_status = himax_distribute_touch_data(buf1, ts_path, ts_status);
    ts_status = himax_err_ctrl(ts, buf1, ts_path, ts_status);
...
static void himax_fb_register(struct work_struct *work)
{
    ...
    I(" %s in\n", __func__);
    ts->fb_notif.notifier_call = hx_drm_notifier_callback;
