...
#define PACKET_MAX_SZ (HX_SPI_6125_MAX_WRITE_SZ + 2)
                ...
                unsigned long event, void *data)
{
    ...
    if (evdata && evdata->data && event == MSM_DRM_EARLY_EVENT_BLANK && ts != NULL &&
        ts->dev != NULL) {
        blank = evdata->data;
        switch (*blank) {
    ...
    if (evdata && evdata->data && event == MSM_DRM_EVENT_BLANK && ts != NULL &&
        ts->dev != NULL) {
        blank = evdata->data;
        switch (*blank) {
