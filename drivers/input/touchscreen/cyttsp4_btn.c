...
static int cyttsp4_btn_open(struct input_dev *input)
{
	...
	tp_log_debug( "%s\n", __func__);
/*< DTS2013031207138 houming 20130312 begin */
/*change condition for waiting */
	pm_runtime_get(dev);
/* DTS2013031207138 houming 20130312 end >*/
...
/* DTS2013031207138 houming 20130312 end >*/
	...
	/* get sysinfo */
	/* < DTS2013081505436  songrongyuan 20130815 begin */
	bd->si = cyttsp4_request_sysinfo(ttsp);
	if (bd->si) {
		rc = cyttsp4_setup_input_device(ttsp);
		if (rc)
	...
	/* DTS2013081505436  songrongyuan 20130815 end > */
