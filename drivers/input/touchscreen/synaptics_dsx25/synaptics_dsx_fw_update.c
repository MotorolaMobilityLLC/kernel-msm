...
static void fwu_compare_partition_tables(void)
{
	...
	tp_log_debug("%s: phyaddr.ui_firmware=%d,"
			"img.phyaddr.ui_firmware=%d\n",__func__,
			fwu->phyaddr.ui_firmware,
			fwu->img.phyaddr.ui_firmware);
	if (fwu->phyaddr.ui_firmware != fwu->img.phyaddr.ui_firmware) {
		fwu->new_partition_table = true;
		return;
	}

	tp_log_debug("%s: phyaddr.ui_config=%d,"
			"img.phyaddr.ui_config=%d\n",__func__,
			fwu->phyaddr.ui_config,
			fwu->img.phyaddr.ui_config);
	...
	if (fwu->flash_properties.has_disp_config) {
		tp_log_debug("%s: has_disp_config:"
				"phyaddr.dp_config=%d,"
				"img.phyaddr.dp_config=%d\n",__func__,
				fwu->phyaddr.dp_config,
				fwu->img.phyaddr.dp_config);
	...
	if (fwu->has_guest_code) {
		tp_log_debug("%s: has_guest_code:"
				"phyaddr.guest_code=%d,"
				"img.phyaddr.guest_code=%d\n",__func__,
				fwu->phyaddr.guest_code,
				fwu->img.phyaddr.guest_code);
...
static int fwu_get_image_firmware_id(unsigned int *fw_id)
{
	...
	tp_log_debug("%s: Image firmware ID=%d !\n",
			__func__,(unsigned int)*fw_id);
...
static enum flash_area fwu_go_nogo(void)
{
	...
	if (image_fw_id > device_fw_id) {
		dev_info(rmi4_data->pdev->dev.parent,
				"%s: Image firmware ID diff with device firmware ID\n",
				__func__);
		flash_area = UI_FIRMWARE;
		goto exit;
	...
	if (image_config_id > device_config_id) {
		dev_info(rmi4_data->pdev->dev.parent,
				"%s: Image config ID diff with device config ID\n",
				__func__);
...
static int fwu_erase_all(void)
{
	...
	if (fwu->bl_version == BL_V7) {
		...
		tp_log_debug("%s:[BL_V7] Erase command written\n",
				__func__);
...
static void synaptics_refresh_configid(void)
{
	...
	/* Get device config ID */
	ret = synaptics_rmi4_reg_read(fwu->rmi4_data,
				fwu->f34_fd.ctrl_base_addr,
				config_id,
				sizeof(config_id));
	if (ret < 0) {
		dev_err(fwu->rmi4_data->pdev->dev.parent,
				"%s:Failed to read device config ID,ret=%d\n",
				__func__,ret);
		fwu->rmi4_data->config_id = 0;
		goto exit;
	}
...
static int fwu_start_reflash(void)
{
	...
	tp_log_debug("%s: Start of reflash process\n",__func__);
	...
	if (fwu->image == NULL) {
		...
		tp_log_debug("%s: Requesting firmware image %s\n",
				__func__, fwu->image_name);
		dev_dbg(rmi4_data->pdev->dev.parent,
...
exit:
	...
	retval = fwu_scan_pdt();
	tp_log_debug("%s:fwu_scan_pdt ,retval=%d",__func__,retval);
	synaptics_refresh_configid();
...
EXPORT_SYMBOL(synaptics_dsx25_fw_updater);
...
static void fwu_startup_fw_update_work(struct work_struct *work)
{
	...
	synaptics_dsx25_fw_updater(NULL);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	retval = synaptics_dsx25_fw_updater(fwu->image);
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to do reflash\n",
				__func__);
...
exit:
	complete(&dsx_fwu_remove_complete);
...
{
	synaptics_dsx_new_function(&fwu_module, true);
...
{
	synaptics_dsx_new_function(&fwu_module, false);

	wait_for_completion(&dsx_fwu_remove_complete);
