...
DECLARE_COMPLETION(dsx_rmidev_remove_complete);
...
exit:
	complete(&dsx_rmidev_remove_complete);
...
{
	synaptics_dsx_new_function(&rmidev_module, true);
...
{
	synaptics_dsx_new_function(&rmidev_module, false);

	wait_for_completion(&dsx_rmidev_remove_complete);
