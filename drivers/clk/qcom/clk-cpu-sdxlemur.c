						...
						unsigned long prate, u8 index)
{
	...
	return mux_div_set_src_div(cpuclk, cpuclk->parent_map[index],
					cpuclk->div);
}
...
static int cpucc_clk_determine_rate(struct clk_hw *hw, struct clk_rate_request *req)
{
	...
	if (rate == clk_hw_get_rate(xo_hw)) {
		...
		cpuclk->div = 1;
		cpuclk->src = cpuclk->parent_map[P_BI_TCXO];
		return 0;
	}
	...
	if (rate <= rrate) {
		...
		req->rate = rrate;
		cpuclk->src = cpuclk->parent_map[P_GPLL0];
		...
		cpuclk->src = cpuclk->parent_map[P_APCS_CPU_PLL];
		div = 1;
	}
					...
					unsigned long prate)
{
	...
	mux_div_get_src_div(cpuclk, &src, &div);

	cpuclk->src = src;
	cpuclk->div = div;

	for (i = 0; i < num_parents; i++) {
		if (src == cpuclk->parent_map[i]) {
...
static const u32 apcs_mux_clk_parent_map_qcs404[] = {0, 4, 5};
