...
static struct clk_vdd_class *gcc_alor_regulators[] = {
	&vdd_cx,
	&vdd_mx,
};
...
static struct clk_rcg2 gcc_pcie_0_aux_clk_src = {
	...
	.clkr.vdd_data = {
		.vdd_classes = gcc_alor_regulators,
		.num_vdd_classes = ARRAY_SIZE(gcc_alor_regulators),
		.num_rate_max = VDD_NUM,
		.rate_max = (unsigned long[VDD_NUM]) {
...
static struct clk_branch gcc_camera_hf_axi_cloned_clk = {
	.halt_reg = 0x26040,
	.halt_check = BRANCH_HALT_SKIP,
	.hwcg_reg = 0x26040,
	.hwcg_bit = 1,
	.clkr = {
		...
		.enable_mask = BIT(0),
		.hw.init = &(const struct clk_init_data) {
			.name = "gcc_camera_hf_axi_cloned_clk",
			.ops = &clk_branch2_ops,
		},
	},
};
...
static struct clk_regmap *gcc_alor_clocks[] = {
	...
	[GCC_CAMERA_HF_AXI_CLONED_CLK] = &gcc_camera_hf_axi_cloned_clk.clkr,
...
static const struct qcom_reset_map gcc_alor_resets[] = {
...
static struct gdsc *gcc_alor_gdscs[] = {
...
static const struct regmap_config gcc_alor_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
...
static const struct qcom_cc_desc gcc_alor_desc = {
	.config = &gcc_alor_regmap_config,
	.clks = gcc_alor_clocks,
	.num_clks = ARRAY_SIZE(gcc_alor_clocks),
	.resets = gcc_alor_resets,
	.num_resets = ARRAY_SIZE(gcc_alor_resets),
	.clk_regulators = gcc_alor_regulators,
	.num_clk_regulators = ARRAY_SIZE(gcc_alor_regulators),
	.gdscs = gcc_alor_gdscs,
	.num_gdscs = ARRAY_SIZE(gcc_alor_gdscs),
};

static const struct of_device_id gcc_alor_match_table[] = {
	{ .compatible = "qcom,alor-gcc" },
	{ }
};
MODULE_DEVICE_TABLE(of, gcc_alor_match_table);

static int gcc_alor_probe(struct platform_device *pdev)
{
	struct regmap *regmap;
	int ret;

	regmap = qcom_cc_map(pdev, &gcc_alor_desc);
	if (IS_ERR(regmap))
	...
	ret = qcom_cc_really_probe(&pdev->dev, &gcc_alor_desc, regmap);
	if (ret) {
...
static void gcc_alor_sync_state(struct device *dev)
{
	qcom_cc_sync_state(dev, &gcc_alor_desc);
}

static struct platform_driver gcc_alor_driver = {
	.probe = gcc_alor_probe,
	.driver = {
		.name = "gcc-alor",
		.of_match_table = gcc_alor_match_table,
		.sync_state = gcc_alor_sync_state,
	},
};

static int __init gcc_alor_init(void)
{
	return platform_driver_register(&gcc_alor_driver);
}
subsys_initcall(gcc_alor_init);

static void __exit gcc_alor_exit(void)
{
	platform_driver_unregister(&gcc_alor_driver);
}
module_exit(gcc_alor_exit);

MODULE_DESCRIPTION("QTI GCC ALOR Driver");
MODULE_LICENSE("GPL");
