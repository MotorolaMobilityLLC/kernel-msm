...
static struct clk_vdd_class *disp_cc_crow_regulators[] = {
	&vdd_cx,
	&vdd_mxa,
};
...
static const struct freq_tbl ftbl_disp_cc_mdss_mdp_clk_src[] = {
	...
	F(400000000, P_DISP_CC_PLL0_OUT_MAIN, 3, 0, 0),
	F(520000000, P_DISP_CC_PLL0_OUT_MAIN, 3, 0, 0),
	{ }
};
...
static struct clk_rcg2 disp_cc_mdss_mdp_clk_src = {
	...
	.clkr.vdd_data = {
		.vdd_classes = disp_cc_crow_regulators,
		.num_vdd_classes = ARRAY_SIZE(disp_cc_crow_regulators),
		.num_rate_max = VDD_NUM,
		.rate_max = (unsigned long[VDD_NUM]) {
			[VDD_LOWER] = 200000000,
			[VDD_LOW] = 342000000,
			[VDD_LOW_L1] = 400000000,
			[VDD_NOMINAL] = 520000000},
	},
};
...
static struct clk_regmap *disp_cc_crow_clocks[] = {
...
static const struct regmap_config disp_cc_crow_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
...
static struct qcom_cc_desc disp_cc_crow_desc = {
	.config = &disp_cc_crow_regmap_config,
	.clks = disp_cc_crow_clocks,
	.num_clks = ARRAY_SIZE(disp_cc_crow_clocks),
	.clk_regulators = disp_cc_crow_regulators,
	.num_clk_regulators = ARRAY_SIZE(disp_cc_crow_regulators),
};

static const struct of_device_id disp_cc_crow_match_table[] = {
	{ .compatible = "qcom,crow-dispcc" },
	{ }
};
MODULE_DEVICE_TABLE(of, disp_cc_crow_match_table);

static int disp_cc_crow_probe(struct platform_device *pdev)
{
	struct regmap *regmap;
	int ret;

	regmap = qcom_cc_map(pdev, &disp_cc_crow_desc);
	if (IS_ERR(regmap))
		return PTR_ERR(regmap);

	ret = qcom_cc_runtime_init(pdev, &disp_cc_crow_desc);
	if (ret)
		return ret;
	...
	ret = qcom_cc_really_probe(pdev, &disp_cc_crow_desc, regmap);
	if (ret) {
...
static void disp_cc_crow_sync_state(struct device *dev)
{
	qcom_cc_sync_state(dev, &disp_cc_crow_desc);
}

static const struct dev_pm_ops disp_cc_crow_pm_ops = {
	SET_RUNTIME_PM_OPS(qcom_cc_runtime_suspend, qcom_cc_runtime_resume, NULL)
	SET_SYSTEM_SLEEP_PM_OPS(pm_runtime_force_suspend,
				pm_runtime_force_resume)
};

static struct platform_driver disp_cc_crow_driver = {
	.probe = disp_cc_crow_probe,
	.driver = {
		.name = "disp_cc-crow",
		.of_match_table = disp_cc_crow_match_table,
		.sync_state = disp_cc_crow_sync_state,
		.pm = &disp_cc_crow_pm_ops,
	},
};

static int __init disp_cc_crow_init(void)
{
	return platform_driver_register(&disp_cc_crow_driver);
}
subsys_initcall(disp_cc_crow_init);

static void __exit disp_cc_crow_exit(void)
{
	platform_driver_unregister(&disp_cc_crow_driver);
}
module_exit(disp_cc_crow_exit);

MODULE_DESCRIPTION("QTI DISP_CC CROW Driver");
