...
//ASUS_BSP +++ Maggie_Lee "parse regulator data"
int inv_parse_regulator(struct device *dev, struct mpu_platform_data *pdata)
{
	int rc;
	struct device_node *np = dev->of_node;
	const char *name;

	if (of_property_read_string(np, "inven,vdd_ana", &name)) {
		dev_err(dev, "Missing inven,vdd_ana.\n");
		return -EINVAL;
	}
	pdata->vdd_ana = regulator_get(dev, name);
	...
	if (of_property_read_string(np, "inven,vcc_i2c", &name)) {
		dev_err(dev, "Missing inven,vcc_i2c.\n");
		return -EINVAL;
	}
	pdata->vdd_i2c = regulator_get(dev, name);
...
int invensense_mpu_parse_dt(struct device *dev, struct mpu_platform_data *pdata)
{
	...
	inv_parse_regulator(dev, pdata);

	//ASUS_BSP +++ Maggie_Lee "moved to inv_parse_regulator"
	#if 0
