...
static ssize_t get_mpu65xx_chip_status(struct device *dev, struct device_attribute *devattr, char *buf)
{
	printk("read_chip_status = %d",mpu_status);
	return sprintf(buf, "%d\n", mpu_status);
}
...
static DEVICE_ATTR(chip_status, S_IRUGO| S_IWUSR, get_mpu65xx_chip_status, NULL);
static DEVICE_ATTR(accel_9250_raw, S_IRUGO|S_IWUSR, read_accel_raw, NULL);

static struct attribute *inv_factory_attributes[] = {
        &dev_attr_chip_status.attr,
        &dev_attr_accel_9250_raw.attr,
};
#endif
//ASUS_BSP --- Maggie_Lee "Support ATD BMMI and calibration"
...
static struct attribute *inv_attributes[
	...
	//ASUS_BSP +++ Maggie_Lee "Support ATD BMMI and calibration"
	#ifdef ASUS_FACTORY_BUILD
	ARRAY_SIZE(inv_factory_attributes) +
	#endif
	//ASUS_BSP --- Maggie_Lee "Support ATD BMMI and calibration"
	1
];
...
static long inv_mpu_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    int value = 0;
    int ret = 0;
    u8 ped_enabled = g_st->chip_config.enable;
    ...
    case IOCTL_SET_PED_SENSITIVITY:
        ...
        if (ped_enabled) {
            ...
            ret = write_be32_key_to_mem(g_st, value*MPU_DEFAULT_PED_PEAK_PARAM, KEY_D_PEDSTD_PEAKTHRSH);
            if (ret) {
                ret |= g_st->set_power_state(g_st, false);
                goto access_failed;
            }
...
msleep(100);
	...
	result = iio_buffer_register(indio_dev, indio_dev->channels, indio_dev->num_channels);
	if (result) {
...
static int inv_mpu_remove(struct i2c_client *client)
{
	...
	mpu_status=0;			//ASUS_BSP +++ Maggie_Lee "Support ATD BMMI"
