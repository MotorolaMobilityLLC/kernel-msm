...
static int slg51000_regulator_is_enabled(struct regulator_dev *rdev)
{
	...
	if (ret < 0) {
		dev_err(chip->dev, "slg51000 Failed to read status register(%d)\n",
			ret);
		return ret;
	}
...
static int slg51000_regulator_init(struct slg51000 *chip)
{
	...
	for (id = 0; id < SLG51000_MAX_REGULATORS; id++) {
		...
		if (ret < 0) {
			dev_err(chip->dev,
				"slg51000 Failed to read the MIN register, id=%d\n",id);
			return ret;
		}

		pr_err("in slg51000_regulator_init id=%d...\n",id);
		...
		case SLG51000_REGULATOR_LDO2:
			...
			if (ret < 0) {
				dev_err(chip->dev,
					"slg51000 Failed to read voltage range of ldo%d\n",
					id + 1);
				return ret;
			}
		...
		case SLG51000_REGULATOR_LDO6:
			...
			if (ret < 0) {
				dev_err(chip->dev,
					"slg51000 Failed to read LDO mode register\n");
				return ret;
			}
		...
		if (IS_ERR(chip->rdev[id])) {
			...
			dev_err(chip->dev,
				"slg51000 Failed to register regulator(%s):%d\n",
...
static irqreturn_t slg51000_irq_handler(int irq, void *data)
{
	...
	for (i = 0; i < SLG51000_MAX_EVT_REGISTER; i++) {
		...
		if (ret < 0) {
			dev_err(chip->dev,
				"slg51000 Failed to read event registers(%d)\n", ret);
			return IRQ_NONE;
		}
	}
		...
		if (!(evt[i][2] & SLG51000_IRQ_ILIM_FLAG_MASK)
		   && (evt[i][0] & SLG51000_EVT_ILIM_FLAG_MASK)) {
			...
			if (evt[i][1] & SLG51000_STA_ILIM_FLAG_MASK)
				dev_warn(chip->dev,
					 "slg51000 Over-current limit(ldo%d)\n", i + 1);
			handled = IRQ_HANDLED;
		}
	}
	   ...
	   && (evt[SLG51000_SCTL_EVT][0] & SLG51000_EVT_HIGH_TEMP_WARN_MASK)) {
		...
		if (evt[SLG51000_SCTL_EVT][1]
		   & SLG51000_STA_HIGH_TEMP_WARN_MASK)
			dev_warn(chip->dev, "slg51000 High temperature warning!\n");
	}
...
static void slg51000_check_crc_and_clear_fault(struct slg51000 *chip)
{
	...
	if (ret < 0) {
		dev_err(chip->dev, "slg51000 Failed to read OTP CRC register\n");
		return;
	}
	...
	if (ret < 0) {
		dev_err(chip->dev, "slg51000 Failed to read Fault log register\n");
		return;
	}
				...
				const struct i2c_device_id *id)
{
	...
	pr_err("enter slg51000_i2c_probe...\n");
	...
	if (cs_gpio > 0) {
		if (!gpio_is_valid(cs_gpio)) {
			dev_err(dev, "slg51000 Invalid chip select pin\n");
			return -EPERM;
		}
		...
		ret = devm_gpio_request_one(dev, cs_gpio, GPIOF_OUT_INIT_HIGH,
					    "slg51000_cs_pin");
		if (ret) {
			dev_err(dev, "slg51000 GPIO(%d) request failed(%d)\n",
				cs_gpio, ret);
			return ret;
		}

		pr_err("slg51000 of_get_named_gpio succeed, cs_gpio=%d\n",cs_gpio);
	...
	if (IS_ERR(chip->regmap)) {
		...
		dev_err(dev, "slg51000 Failed to allocate register map: %d\n",
			error);
		return error;
	}
	...
	if (ret < 0) {
		dev_err(chip->dev, "slg51000 Failed to init regulator(%d)\n", ret);
		return ret;
	}
	...
	if (chip->chip_irq) {
		ret = devm_request_threaded_irq(dev, chip->chip_irq, NULL,
						...
						"slg51000-irq", chip);
		if (ret != 0) {
			dev_err(dev, "slg51000 Failed to request IRQ: %d\n",
				chip->chip_irq);
			return ret;
		}
		pr_err("slg51000 devm_request_threaded_irq succeed, irq=%d\n",chip->chip_irq);
	} else {
		dev_warn(dev, "slg51000 No IRQ configured\n");
	}

	pr_err("exit slg51000_i2c_probe...\n");
