...
#define UFSHID_QUERY_DESC_DEVICE_MAX_SIZE		0x5F
...
#define HID_DEBUG(hid, msg, args...)					\
	do { if (hid->hid_debug)					\
		pr_err("%40s:%3d [%01d%02d%02d] " msg "\n",		\
		       __func__, __LINE__,				\
		       hid->hid_trigger,				\
		       atomic_read(&hid->hba->dev->power.usage_count),\
		       hid->hba->clk_gating.active_reqs, ##args);	\
	} while (0)
...
void ufshid_set_init_state(struct ufshid_dev *hid);
void ufshid_init(struct ufshid_dev *hid);
void ufshid_reset(struct ufshid_dev *hid);
void ufshid_reset_host(struct ufshid_dev *hid);
void ufshid_remove(struct ufshid_dev *hid);
int ufshid_suspend(void);
int ufshid_resume(void);
void ufshid_on_idle(struct ufshid_dev *hid);
void ufshid_set_vops(struct ufshid_dev *hid);
int ufshid_read_dev_desc(struct ufs_hba *hba, u8 selector);
