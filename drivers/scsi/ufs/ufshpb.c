...
#endif
	ret = ufshpb_blk_execute_rq_nowait_mimic(q, req, ufshpb_map_req_compl_fn);
	if (ret) {
		scsi_device_put(sdev);
		return ret;
	}
...
static inline void ufshpb_purge_active_subregion(struct ufshpb_lu *hpb,
						 ...
						 int state)
{
	if (unlikely(state == HPB_SRGN_UNUSED)) {
...
void ufshpb_get_lu_info(struct ufsf_feature *ufsf, int lun, u8 *unit_buf)
{
	...
	INFO_MSG("LUN(%d) [06] bLuQueueDepth (%d)", lun,
...
static void ufshpb_stat_reset(struct ufshpb_lu *hpb)
{
						   ...
						   const char *buf, size_t cnt)
{
	...
	if ((int)throttle_map_req < 0 || (int)throttle_map_req > hpb->qd)
		return -EINVAL;
						   ...
						   const char *buf, size_t cnt)
{
	...
	if ((int)throttle_pre_req < 0 || (int)throttle_pre_req > hpb->qd)
		return -EINVAL;
...
static ssize_t ufshpb_sysfs_map_req_show(struct ufshpb_lu *hpb, char *buf)
{
	...
	INFO_MSG("rb_noti %lld ACT %lld INACT %lld map_req_count %lld map_compl_count %lld",
		 rb_noti_cnt, rb_active_cnt, rb_inactive_cnt,
		 map_req_cnt, map_compl_cnt);
