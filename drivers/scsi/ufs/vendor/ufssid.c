...
void ufssid_get_dev_info(struct ufsf_feature *ufsf, u8 *desc_buf)
{
	...
	spec_version = LI_EN_16(&desc_buf[DEVICE_DESC_PARAM_SID_VER]);
...
void ufssid_prep_fn(struct ufsf_feature *ufsf, struct ufshcd_lrb *lrbp)
{
	...
	if (sid->stream_id_enabled && cmd->request->bio) {
		if (cmd->cmnd[0] == WRITE_10)
			cmd->cmnd[6] = cmd->request->bio->bi_write_hint;
		if (cmd->cmnd[0] == WRITE_16)
			cmd->cmnd[14] = cmd->request->bio->bi_write_hint;
	}
}
						    ...
						    size_t count)
{
	...
	ret = ufsf_query_flag(hba, opcode, QUERY_FLAG_IDN_STREAM_ID_EN, 0,
