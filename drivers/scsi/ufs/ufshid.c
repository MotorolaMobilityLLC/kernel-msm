...
inline int ufshid_get_state(struct ufshid_dev *hid)
{
	return atomic_read(&hid->hid_state);
}

inline void ufshid_set_state(struct ufshid_dev *hid, int state)
{
...
{
	enum UFSHID_STATE cur_state = ufshid_get_state(hid);
...
static int ufshid_read_attr(struct ufshid_dev *hid, u8 idn, u32 *attr_val)
{
	...
	ret = ufshcd_query_attr(hba, UPIU_QUERY_OPCODE_READ_ATTR, idn, 0, UFSHID_SELECTOR,
				    attr_val);
	if (ret) {
...
static int ufshid_write_attr(struct ufshid_dev *hid, u8 idn, u32 val)
{
	...
	ret = ufshcd_query_attr(hba, UPIU_QUERY_OPCODE_WRITE_ATTR, idn, 0, UFSHID_SELECTOR,
				    &val);
	if (ret) {
...
struct ufshid_dev *ufshid_get_dev_info(struct ufshid_dev *hid_dev)
{
	...
	hid_dev = kzalloc(sizeof(struct ufshid_dev), GFP_KERNEL);
	if (!hid_dev) {
	...
	return hid_dev;
err_out:
	//ufshid_set_state(hid_dev, HID_FAILED);
	return NULL;
}

int ufshid_read_dev_desc(struct ufs_hba *hba, u8 selector)
{
	u8 desc_buf[UFSHID_QUERY_DESC_DEVICE_MAX_SIZE];
	int ret;
	int device_des_size = UFSHID_QUERY_DESC_DEVICE_MAX_SIZE;

	ret = ufshcd_query_descriptor_retry(hba, UPIU_QUERY_OPCODE_READ_DESC ,QUERY_DESC_IDN_DEVICE, 0, selector,
			     desc_buf, &device_des_size);
	if (ret)
		return ret;
...
//	ufshid_get_dev_info(hid_dev, desc_buf);
...
static int ufshid_wait_for_doorbell_clr(struct ufs_hba *hba, u64 wait_timeout_us )
{
	unsigned long flags;
	int ret = 0;
	u32 tm_doorbell;
	...
	start = ktime_get();
	do {
		ufs_intr_en = ufshcd_readl(hba, REG_INTERRUPT_ENABLE);
		if ( ( ufs_intr_en & UTP_TRANSFER_REQ_COMPL ) == 0 ){
			ret = -EBUSY;
			goto out;
		}
...
static void ufshid_scsi_block_requests(struct ufs_hba *hba)
{
	if (atomic_inc_return(&hba->scsi_block_reqs_cnt) == 1)
		scsi_block_requests(hba->host);
}

static void ufshid_scsi_unblock_requests(struct ufs_hba *hba)
{
...
//	struct ufs_hba *hba = hid->ufsf->hba;
	...
	ufshid_scsi_block_requests(hba);
	/* wait for all the outstanding requests to finish */
	ufshid_wait_for_doorbell_clr(hba, U64_MAX);
	spin_lock_irqsave(hba->host->host_lock, flags);
	...
	spin_unlock_irqrestore(hba->host->host_lock, flags);
	ufshid_scsi_unblock_requests(hba);
...
void ufshid_init(struct ufshid_dev *hid)
{
	...
	if (ret) {
		...
		kfree(hid);
		ufshid_set_state(hid, HID_FAILED);
		return;
	}
...
void ufshid_reset_host(struct ufshid_dev *hid)
{
	...
	ufshid_set_state(hid, HID_RESET);
...
void ufshid_reset(struct ufshid_dev *hid)
{
	...
	if(ufshid_get_state(hid) != HID_RESET)
		return ;
...
#if 0
static int ufshid_device_reset(void)
{
	struct ufshid_dev *hid = hid_local;
...
int ufshid_suspend(void)
{
	...
	if (hid->hid_trigger){
		...
		ufshid_set_state(hid, HID_SUSPEND);
...
int ufshid_resume(void)
{
	...
	if (hid->hid_trigger){
		...
		ufshid_set_state(hid, HID_PRESENT);
		schedule_delayed_work(&hid->hid_trigger_work, msecs_to_jiffies(HID_TRIGGER_WORKER_DELAY_MS_DEFAULT));
	}
