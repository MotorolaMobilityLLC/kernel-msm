 ...
 * UFSHPB DEBUG
 */
#define hpb_err(hba, msg, args...) dev_err(hba?hba->dev:NULL, msg, ##args)	

#define hpb_info(hba, msg, args...) dev_info(hba?hba->dev:NULL, msg, ##args)
		...
		struct ufshcd_lrb *lrbp, unsigned long long ppn)
{
	...
	if (hpb->hpb_ver == UFSHPB_VER_20)
		cmd[1] = 0xe0 | (lrbp->cmd->cmnd[1] & 0x1f);
	else
		cmd[15] = 1;

	memcpy(lrbp->cmd->cmnd, cmd, UFS_CDB_SIZE);
	memcpy(lrbp->ucd_req_ptr->sc.cdb, cmd, UFS_CDB_SIZE);
}
...
{
	return (lrbp->utr_descriptor_ptr->header.dword_0 & UTP_REQ_DESC_CRYPTO_ENABLE_CMD);
}
		...
		struct ufshpb_map_req *map_req)
{
	...
	hpb_dbg(hpb->hba, "start=%llu, issue=%llu, endio=%llu, endio=%llu\n",
			map_req->RSP_workq_enter1 - map_req->RSP_start,
			map_req->RSP_issue - map_req->RSP_workq_enter1,
		...
		struct request_queue *q, struct ufshpb_map_req *map_req)
{
	...
	ufshpb_set_read_buf_cmd(cmd, map_req->region, map_req->subregion,
        ...
        if (IS_ERR(rq)) {
                hpb_dbg(hpb->hba, "Can't allocate request %d", PTR_ERR(rq));
                return PTR_ERR(rq);
        }
	...
	blk_execute_rq_nowait(q, NULL, rq, 1, ufshpb_map_req_compl_fn);
		...
		struct ufshpb_rsp_info *rsp_info)
{
	...
	map_req->RSP_workq_enter1 = rsp_info->RSP_workq_enter;
		...
		struct ufshpb_rsp_info *rsp_info)
{
	...
	if (ret) {
		hpb_err(hpb->hba, "region evict/load failed. ret %d\n", ret);
	...
	for (iter = 0; iter < rsp_info->active_cnt; iter++) {
		...
		ret = ufshpb_set_map_req(hpb, region, subregion,
						cp->mctx, rsp_info);
		if (ret) {
			hpb_err(hpb->hba, "ufshpb_set_map_req error %d\n", ret);
			goto wakeup_ee_worker;
		}
	}
	return;
...
static bool ufshpb_is_fw_support_hpb(struct ufs_hba *hba)
{
	...
	if (!desc_buf)
		return ret_val;
	if(ufshcd_query_descriptor_retry(hba,UPIU_QUERY_OPCODE_READ_DESC,
...
static inline int ufshpb_autopm_get_device(struct ufshpb_lu *hpb)
{
	if (unlikely(!hpb) || unlikely(!hpb->hba) ||
	    unlikely(!hpb->hba->sdev_ufs_lu[hpb->lun]))
		return -ENODEV;
	return scsi_autopm_get_device(hpb->hba->sdev_ufs_lu[hpb->lun]);
}
static inline void ufshpb_autopm_put_device(struct ufshpb_lu *hpb)
{
	if (unlikely(!hpb) || unlikely(!hpb->hba) ||
	    unlikely(!hpb->hba->sdev_ufs_lu[hpb->lun]))
		return;
	scsi_autopm_put_device(hpb->hba->sdev_ufs_lu[hpb->lun]);
}
static void ufshpb_work_handler(struct work_struct *work)
{
	...
	if (!list_empty(&hpb->lh_subregion_req) &&
	    ufshpb_autopm_get_device(hpb) >= 0) {
...
static void ufshpb_retry_work_handler(struct work_struct *work)
{
	...
	if (ufshpb_autopm_get_device(hpb) < 0) {
		return;
	}
...
static void ufshpb_map_work_fn(struct work_struct *work)
{
	struct ufshpb_lu *hpb = container_of(work, struct ufshpb_lu, ufshpb_map_work);
	struct ufshpb_rsp_info *rsp_info;
	unsigned long flags;
	...
	while (1) {
		...
		rsp_info->RSP_workq_enter = ktime_to_ns(ktime_get());
		...
		struct ufshpb_lu_desc *lu_desc, int lun)
{
	...
	if (lu_desc->lu_hpb_enable) {
		hpb_info(hba, "LUN(%d) [0A] bLogicalBlockSize %d\n",
				lun, lu_desc->lu_logblk_size);
		hpb_info(hba, "LUN(%d) [0B] qLogicalBlockCount %llu\n",
				lun, lu_desc->lu_logblk_cnt);
		hpb_info(hba, "LUN(%d) [03] bLuEnable %d\n",
				lun, logical_buf[UNIT_DESC_PARAM_LU_ENABLE]);
		hpb_info(hba, "LUN(%d) [06] bLuQueueDepth %d\n",
				lun, lu_desc->lu_queue_depth);
		hpb_info(hba, "LUN(%d) [23:24] wLUMaxActiveHPBRegions %d\n",
				lun, lu_desc->lu_max_active_hpb_regions);
		hpb_info(hba, "LUN(%d) [25:26] wHPBPinnedRegionStartIdx %d\n",
				lun, lu_desc->hpb_pinned_region_startidx);
		hpb_info(hba, "LUN(%d) [27:28] wNumHPBPinnedRegions %d\n",
		...
		struct ufshpb_func_desc *desc)
{
	...
	pr_info("UFSHPB: Version = %.2x.%.2x, Driver Version = %.2x.%.2x or %.2x.%.2x\n",
			GET_BYTE_1(desc->hpb_ver),
			GET_BYTE_0(desc->hpb_ver),
			GET_BYTE_1(UFSHPB_VER_10),
			GET_BYTE_0(UFSHPB_VER_10),
			GET_BYTE_1(UFSHPB_VER_20),
			GET_BYTE_0(UFSHPB_VER_20));

	if (desc->hpb_ver != UFSHPB_VER_10 && desc->hpb_ver != UFSHPB_VER_20)
		return -ENODEV;
	return 0;
}
...
void ufshcd_init_hpb_toshiba(struct ufs_hba *hba)
{
	...
	INIT_DELAYED_WORK(&hba->ufshpb_init_work, ufshpb_init_handler_toshiba);
}
#else
void ufshcd_init_hpb_toshiba()
{
}
#endif
