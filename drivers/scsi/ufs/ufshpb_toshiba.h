...
#ifndef _UFSHPB_TOSHIBA_H_
#define _UFSHPB_TOSHIBA_H_
...
/* Version info*/
#define UFSHPB_VER_10				0x0100
#define UFSHPB_VER_20				0x0200
...
#define UFSHPB_QUERY_DESC_DEVICE_MAX_SIZE		0x59
#define UFSHPB_QUERY_DESC_CONFIGURAION_MAX_SIZE		0xE6
#define UFSHPB_QUERY_DESC_UNIT_MAX_SIZE			0x2D
#define UFSHPB_QUERY_DESC_GEOMETRY_MAX_SIZE		0x57
...
/* Parameter Macros */
#define HPB_DEV(h)	(((h)&&((h)->hba))?((h)->hba->dev):NULL)
...
#define HPB_RSP_RESET_REGION_INFO			0x02
...
struct ufshpb_rsp_active_entry {
	u16 region;
	u16 subregion;
};

struct ufshpb_rsp_inactive_entry {
	u16 region;
};
...
struct ufshpb_rsp_info {
	...
	int active_cnt;
	int inactive_cnt;
	struct ufshpb_rsp_active_entry active_list[MAX_INACTIVE_NUM];
	struct ufshpb_rsp_inactive_entry inactive_list[MAX_ACTIVE_NUM];

	__u64 RSP_start;
	__u64 RSP_workq_enter;
...
struct ufshpb_map_req {
	...
	__u64 RSP_workq_enter1; // map work sched time
	__u64 RSP_issue; // issue scsi cmd
	__u64 RSP_endio;
	__u64 RSP_workq_enter2;
...
struct ufshpb_lu {
	...
	struct work_struct ufshpb_map_work;
	struct workqueue_struct *ufshpb_map_work_q;
	struct bio_vec bvec[MAX_BVEC_SIZE];
...
#else
void ufshcd_init_hpb(void);
#endif
void ufshpb_init_handler_toshiba(struct work_struct *work);
