...
void pcie_dump_phy_reg(struct msm_pcie_dev_t *dev)
{
	int i;
	int control_offset[2] = {0x60, 0x70};
	PCIE_DBG(dev, "Dumping phy reg for RC%d\n", dev->rc_idx);
	for (i = 0; i < 2; i++) {
		write_phy(dev->phy, PCIE_PHY_TEST_CONTROL, control_offset[i]);
		PCIE_DBG(dev, "TEST_CONTROL(0x%04x)=0x%08x\n",
			PCIE_PHY_TEST_CONTROL, readl_relaxed(dev->phy + PCIE_PHY_TEST_CONTROL));
		PCIE_DBG(dev, "BUS_0_STATUS(0x%04x)=0x%08x\n",
			PCIE_PHY_DEBUG_BUS_0_STATUS, readl_relaxed(dev->phy + PCIE_PHY_DEBUG_BUS_0_STATUS));
		PCIE_DBG(dev, "BUS_1_STATUS(0x%04x)=0x%08x\n",
			PCIE_PHY_DEBUG_BUS_1_STATUS, readl_relaxed(dev->phy + PCIE_PHY_DEBUG_BUS_1_STATUS));
		PCIE_DBG(dev, "BUS_2_STATUS(0x%04x)=0x%08x\n",
			PCIE_PHY_DEBUG_BUS_2_STATUS, readl_relaxed(dev->phy + PCIE_PHY_DEBUG_BUS_2_STATUS));
		PCIE_DBG(dev, "BUS_3_STATUS(0x%04x)=0x%08x\n",
			PCIE_PHY_DEBUG_BUS_3_STATUS, readl_relaxed(dev->phy + PCIE_PHY_DEBUG_BUS_3_STATUS));
	}
	pcie_phy_dump(dev);
}
