...
enum qpnp_adc_scale_fn_type {
	...
	SCALE_PMI_CHG_TEMP,
	SCALE_XOTHERM_EPSONFA,
	SCALE_THERM_100K_PULLUP_TDKNTC,
	SCALE_NONE,
};
...
/**
 * qpnp_adc_epsonfa_therm() - Scales the pre-calibrated digital output
 *		of an ADC to the ADC reference and compensates for the
 *		gain and offset. Returns the temperature of the epsonfa xo therm in mili
		degC.
 * @dev:	Structure device for qpnp vadc
 * @adc_code:	pre-calibrated digital ouput of the ADC.
 * @adc_prop:	adc properties of the pm8xxx adc such as bit resolution,
...
/**
 * qpnp_adc_scale_therm_tdkntc() - Scales the pre-calibrated digital output
 *		of an ADC to the ADC reference and compensates for the
 *		gain and offset. Returns the temperature of the therm in degC.
 *		It uses a mapping table computed for a 100K pull-up tdkntc.
 *		Pull-up2 is an internal pull-up on the AMUX of 100K.
 * @dev:	Structure device for qpnp vadc
 * @adc_code:	pre-calibrated digital ouput of the ADC.
 * @adc_prop:	adc properties of the pm8xxx adc such as bit resolution,
...
int32_t qpnp_adc_scale_therm_tdkntc(struct qpnp_vadc_chip *chip,
		...
		struct qpnp_vadc_result *adc_chan_result);
/**
 * qpnp_adc_tm_scale_therm_voltage_tdkntc() - Performs reverse calibration
 *		and converts the given ADC code to temperature for
 *		thermistor channels using 100k pull-up tdkntc.
 * @dev:	Structure device for qpnp vadc
...
/**
 * qpnp_adc_tm_scale_voltage_therm_tdkntc() - Performs reverse calibration
...
static inline int32_t qpnp_adc_epsonfa_therm(struct qpnp_vadc_chip *chip,
		int32_t adc_code,
		const struct qpnp_adc_properties *adc_properties,
		const struct qpnp_vadc_chan_properties *chan_properties,
		struct qpnp_vadc_result *adc_chan_result)
...
static inline int32_t qpnp_adc_scale_therm_tdkntc(struct qpnp_vadc_chip *chip,
		int32_t adc_code,
		const struct qpnp_adc_properties *adc_properties,
		const struct qpnp_vadc_chan_properties *chan_properties,
		struct qpnp_vadc_result *adc_chan_result)
...
static inline int32_t qpnp_adc_tm_scale_therm_voltage_tdkntc(struct qpnp_vadc_chip *chip,
				struct qpnp_adc_tm_config *param)
{ return -ENXIO; }

static inline int32_t qpnp_adc_tm_scale_voltage_therm_tdkntc(struct qpnp_vadc_chip *chip,
					uint32_t reg, int64_t *result)
{ return -ENXIO; }
